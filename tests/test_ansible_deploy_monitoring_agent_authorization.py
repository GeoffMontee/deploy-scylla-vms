...
import pytest
import test_ansible_deploy_manager_agent_reconciliation as manager_fixture
from test_provider_source import CLUSTER_UUID
from test_show import _run as _run_show
from test_terraform_plan_checkpoint import OPERATION_ID

import scylla_vms.ansible.deploy_monitoring_agent_authorization as authorization_module
...
from scylla_vms.ansible.monitoring_agent import MONITORING_AGENT_PACKAGES
...
_PRIVATE_PATH = "/private/operator/monitoring-agent-authorization.json"
_SECRET = "obviously-fake-monitoring-agent-authorization-secret"
_PROMPT = "APPROVE node exporter token at 10.0.0.11:9100?"


def _proof(
    method: DeployMonitoringAgentApprovalMethod = (
        DeployMonitoringAgentApprovalMethod.INTERACTIVE
    ),
) -> DeployMonitoringAgentAuthorizationProof:
    return DeployMonitoringAgentAuthorizationProof(
...
def _call(prepared, proof: DeployMonitoringAgentAuthorizationProof):
    with ClusterLock(prepared.paths, "deploy", 0) as lock:
        return authorize_deploy_monitoring_agent(
...
def _record(prepared):
    with ClusterLock(prepared.paths, "deploy", 0) as lock:
        return DeployMonitoringAgentAuthorizationStore(
...
@pytest.mark.parametrize(
    ...
    (
        DeployMonitoringAgentApprovalMethod.INTERACTIVE,
        DeployMonitoringAgentApprovalMethod.CLI_YES,
    ),
)
def test_exact_scope_is_immutable_redacted_and_show_validated(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    method: DeployMonitoringAgentApprovalMethod,
) -> None:
    signature = inspect.signature(authorize_deploy_monitoring_agent)
    ...
    path = deploy_monitoring_agent_authorization_path(prepared.paths, OPERATION_ID)
    ...
    assert reused.artifact_state is (
        DeployMonitoringAgentAuthorizationArtifactState.REUSED
    )
    ...
    assert report.playbook == "monitoring-agent"
    ...
    assert report.package_count == len(MONITORING_AGENT_PACKAGES) == 1
    assert report.service_policy is (
        DeployMonitoringAgentServicePolicy.DISABLED_INACTIVE
    )
    assert report.listen_policy is DeployMonitoringAgentListenPolicy.NOT_STARTED
    ...
    for index, scope in enumerate(record.scopes, start=1):
        ...
        assert scope.mapping_sequence == 17
        assert scope.playbook == "monitoring-agent"
        assert scope.target_role == "scylla"
        assert scope.target_stable_id == expected_ids[index - 1]
        assert scope.service_policy is (
            DeployMonitoringAgentServicePolicy.DISABLED_INACTIVE
        )
        assert scope.listen_policy is DeployMonitoringAgentListenPolicy.NOT_STARTED
        assert not any(
            (
    ...
    assert (
        deploy_monitoring_agent_authorization_id_from_filename(path.name)
        == OPERATION_ID
    )
    assert (
        deploy_monitoring_agent_authorization_id_from_filename(f"uppercase-{path.name}")
        is None
    )
...
def test_refuses_missing_denied_destructive_and_narrow_proofs(
    ...
    cases = (
        (DeployMonitoringAgentAuthorizationProof(), "approval is required"),
        (
            DeployMonitoringAgentAuthorizationProof(
                approval_method=DeployMonitoringAgentApprovalMethod.INTERACTIVE,
                approved=False,
            ),
            "approval was denied",
        ),
        (
            DeployMonitoringAgentAuthorizationProof(
                approval_method=DeployMonitoringAgentApprovalMethod.CLI_YES,
                approved=True,
    ...
    assert len(runner.specs) == process_count
    assert not deploy_monitoring_agent_authorization_path(
        prepared.paths, OPERATION_ID
    ).exists()
...
def test_refuses_wrong_role_incomplete_scope_install_gate_and_payload_tamper(
    ...
    with ClusterLock(prepared.paths, "deploy", 0) as lock:
        ...
        install_entry = (
            context.manager.authorization_context.install.evidence.record.entries[0]
        )
        object.__setattr__(install_entry, "service_inactive", False)
        with pytest.raises(StateConflictError, match=r"Scylla-install.*gate"):
            _derive_authorization_scopes(context, package)
        object.__setattr__(install_entry, "service_inactive", True)

        original_builder = authorization_module.build_monitoring_agent_payload

        def tampered_builder(*args, **kwargs):
            payload = original_builder(*args, **kwargs)
            payload["listen_policy"] = "private"
            return payload
        ...
        with pytest.raises(StateConflictError, match="no-listen policy"):
            _derive_authorization_scopes(context, package)
    assert len(runner.specs) == process_count
...
def test_refuses_missing_reconciliation_drift_later_state_and_changed_proof(
    ...
    assert runner.specs is not None
    process_count = len(runner.specs)
    post_path = deploy_post_manager_agent_reconciliation_path(
        prepared.paths, OPERATION_ID
    )
    post_bytes = post_path.read_bytes()
    post_path.unlink()
    with pytest.raises(StateConflictError, match="requires post-manager-agent"):
    ...
    later = prepared.paths.operations / (
        f"{OPERATION_ID}.ansible-deploy-monitoring-agent-execution.json"
    )
    later.write_text("{}\n", encoding="utf-8")
    later.chmod(0o600)
    ...
    with pytest.raises(StateConflictError, match="changed"):
        _call(
            prepared,
            _proof(DeployMonitoringAgentApprovalMethod.CLI_YES),
        )
    assert len(runner.specs) == process_count


def test_refuses_source_drift_store_tamper_and_unsafe_path(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
