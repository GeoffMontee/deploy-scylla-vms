...
import pytest
from test_ansible_deploy_authorization import (
    _call as _authorize,
)
from test_ansible_deploy_authorization import (
    _interactive,
...
from test_ansible_deploy_host_evidence import (
    _call as _collect_host_evidence,
)
from test_ansible_deploy_host_evidence import (
...
class BaseOsRunner:
    ...
    def _result(self, limit: tuple[str, ...]) -> ProcessResult:
        ...
        for index, logical_id in enumerate(limit):
            changed = self.mode in {"changed", "reboot"}
            reboot_required = self.mode == "reboot"
            status = (
                "reboot-required"
                if reboot_required
                else "changed"
...
def _prepared(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    _collect_host_evidence(
        prepared,
        HostEvidenceRunner(inventory),
        executables,
        toolchain,
    )
    _replace_host_evidence(
...
def _records(prepared):
    with ClusterLock(prepared.paths, "deploy", 0) as lock:
        execution = DeployBaseOsExecutionStore(
        ...
        evidence_store = DeployBaseOsEvidenceStore(prepared.paths, OPERATION_ID)
        evidence = (
...
def test_exact_authorized_scope_command_variables_success_and_zero_call_reentry(
    ...
    assert tuple(inspect.signature(execute_deploy_base_os).parameters) == (
        "state_root",
        "cluster_name",
        "operation_id",
        "lock",
    ...
    assert (
        evidence.record.schema_version == ANSIBLE_DEPLOY_BASE_OS_EVIDENCE_SCHEMA_VERSION
    )
    assert report.execution_state is DeployBaseOsExecutionState.SUCCEEDED
    assert report.authorization_consumed
    assert report.invocation_count == report.scope_count == 1
    assert report.stable_id_count == 1
    ...
    persisted = deploy_base_os_execution_path(prepared.paths, OPERATION_ID).read_text(
        encoding="utf-8"
    ) + deploy_base_os_evidence_path(prepared.paths, OPERATION_ID).read_text(
        encoding="utf-8"
    )
    ...
    with pytest.raises(StateConflictError, match="prior execution"):
        _authorize(prepared, _interactive())
...
@pytest.mark.parametrize(
    ("mode", "changed", "reboot"),
    (("no-change", 0, 0), ("changed", 1, 0), ("reboot", 1, 1)),
)
...
def test_exit_zero_malformed_wrong_missing_extra_duplicate_and_oversize_are_no_retry(
    ...
    execution, evidence = _records(prepared)
    assert execution.record.state is DeployBaseOsExecutionState.MALFORMED_RESULT
...
@pytest.mark.parametrize(
    ...
    (
        ("nonzero", DeployBaseOsExecutionState.FAILED, True),
        ("unreachable", DeployBaseOsExecutionState.UNREACHABLE, True),
        ("timeout", DeployBaseOsExecutionState.TIMED_OUT, False),
        ("interrupted", DeployBaseOsExecutionState.INTERRUPTED, False),
        ("non-utf8", DeployBaseOsExecutionState.MALFORMED_RESULT, False),
    ),
)
...
def test_prepared_recovery_and_started_consumption_precede_call(
    ...
    prepared, inventory, executables, toolchain = _prepared(tmp_path, monkeypatch)
    original = DeployBaseOsExecutionStore.write_locked
    refused = False

    def fail_started(self, record, **kwargs):
        nonlocal refused
        if record.state is DeployBaseOsExecutionState.STARTED and not refused:
            refused = True
            raise StatePersistenceError("simulated safe pre-invocation refusal")
        return original(self, record, **kwargs)

    monkeypatch.setattr(DeployBaseOsExecutionStore, "write_locked", fail_started)
    ...
    execution, evidence = _records(prepared)
    assert execution.record.state is DeployBaseOsExecutionState.PREPARED
    ...
    monkeypatch.setattr(DeployBaseOsExecutionStore, "write_locked", original)
    observed: list[DeployBaseOsExecutionState] = []

    def inspect_started() -> None:
        value = json.loads(
            deploy_base_os_execution_path(prepared.paths, OPERATION_ID).read_text(
                encoding="utf-8"
            )
        )
    ...
    assert report.execution_state is DeployBaseOsExecutionState.SUCCEEDED
    assert observed == [DeployBaseOsExecutionState.STARTED]
...
def test_post_invocation_persistence_failure_is_permanent_manual_recovery(
    ...
    monkeypatch.setattr(DeployBaseOsEvidenceStore, "append_locked", fail_evidence)
    ...
    execution, evidence = _records(prepared)
    assert execution.record.state is DeployBaseOsExecutionState.STARTED
    assert execution.record.authorization_consumed
    assert evidence is None
...
def test_lock_symlink_permissions_ambiguity_and_caller_scope_refusal(
    ...
    assert runner.specs == []
    assert "limit" not in inspect.signature(execute_deploy_base_os).parameters
    assert "variables" not in inspect.signature(execute_deploy_base_os).parameters
    assert "environment" not in inspect.signature(execute_deploy_base_os).parameters
    assert "playbook" not in inspect.signature(execute_deploy_base_os).parameters
