...
import pytest
from test_ansible_deploy_manager_agent_execution import (
    ManagerAgentRunner,
    _prepared,
    _prepared_multi,
)
from test_ansible_deploy_manager_agent_execution import _call as _execute_agent
from test_provider_source import CLUSTER_UUID
from test_show import _run as _run_show
from test_terraform_plan_checkpoint import OPERATION_ID

import scylla_vms.ansible.deploy_manager_agent_reconciliation as reconciliation_module
...
_PRIVATE_PATH = "/private/operator/post-manager-agent.json"
_SECRET = "obviously-fake-post-manager-agent-secret"
...
def _complete(
    ...
    runner = ManagerAgentRunner(mode=mode)
...
def _call(prepared):
    with ClusterLock(prepared.paths, "deploy", 0) as lock:
        return reconcile_deploy_manager_agent_result(
...
def _record(prepared):
    with ClusterLock(prepared.paths, "deploy", 0) as lock:
        return DeployPostManagerAgentReconciliationStore(
...
def test_success_reuse_mapping_next_gate_redaction_and_show(
    ...
    signature = inspect.signature(reconcile_deploy_manager_agent_result)
    ...
    path = deploy_post_manager_agent_reconciliation_path(prepared.paths, OPERATION_ID)
    ...
    assert report.artifact_state is DeployPostManagerAgentArtifactState.CREATED
    assert reused.artifact_state is DeployPostManagerAgentArtifactState.REUSED
    ...
    agent = next(step for step in record.steps if step.mapping_sequence == 16)
    next_step = next(step for step in record.steps if step.mapping_sequence == 17)
    assert agent.playbook == "manager-agent"
    assert agent.status is DeployBaseOsReconciledStepStatus.SUCCEEDED
    assert agent.evidence_state == "manager-agent-bound"
    assert agent.evidence_digest == record.evidence_set_digest
    assert not agent.blockers
    assert report.next_mapping_sequence == next_step.mapping_sequence == 17
    assert report.next_playbook == next_step.playbook == "monitoring-agent"
    assert (
    ...
    assert tuple(step.mapping_sequence for step in advanced) == (17,)
    ...
    assert (
        deploy_post_manager_agent_reconciliation_id_from_filename(path.name)
        == OPERATION_ID
    )
    assert (
        deploy_post_manager_agent_reconciliation_id_from_filename(
...
def test_multi_target_complete_order_and_counts(
    ...
    agent = next(step for step in stored.record.steps if step.mapping_sequence == 16)
...
def test_refuses_missing_failed_uncertain_tamper_drift_and_later_history(
    ...
    with pytest.raises(AnsibleError):
        _execute_agent(
            prepared,
            ManagerAgentRunner(mode="failed"),
            executables,
            toolchain,
        )
    ...
    evidence_path = prepared.paths.operations / (
        f"{OPERATION_ID}.ansible-deploy-manager-agent-evidence.json"
    )
    ...
    entries[0]["auth_token_configured"] = True
