"""Immutable authorization for mapped deploy ``monitoring-agent`` scopes.

This internal owner revalidates the exact post-manager-agent reconciliation,
derives the complete canonical Scylla node-exporter install scope, and persists
only an unconsumed ordinary approval checkpoint. It does not execute Ansible,
configure an exporter, open a listener, or advance the common journal.
"""
...
from scylla_vms.ansible.deploy_manager_agent_reconciliation import (
    ANSIBLE_DEPLOY_POST_MANAGER_AGENT_RECONCILIATION_SCHEMA_VERSION,
    DeployPostManagerAgentReconciliationStore,
    StoredDeployPostManagerAgentReconciliation,
)
from scylla_vms.ansible.deploy_manager_agent_reconciliation import (
...
ANSIBLE_DEPLOY_MONITORING_AGENT_AUTHORIZATION_PROOF_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-monitoring-agent-authorization-proof/v1"
)
ANSIBLE_DEPLOY_MONITORING_AGENT_AUTHORIZATION_SCOPE_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-monitoring-agent-authorization-scope/v1"
)
ANSIBLE_DEPLOY_MONITORING_AGENT_PACKAGE_PROVENANCE_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-monitoring-agent-package-provenance/v1"
)
ANSIBLE_DEPLOY_MONITORING_AGENT_AUTHORIZATION_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-monitoring-agent-authorization/v1"
)
ANSIBLE_DEPLOY_MONITORING_AGENT_AUTHORIZATION_REPORT_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-monitoring-agent-authorization-report/v1"
)
DEPLOY_MONITORING_AGENT_AUTHORIZATION_FILENAME_SUFFIX = (
    ".ansible-deploy-monitoring-agent-authorization.json"
)
...
_STAGE = "post-manager-agent-monitoring-agent"
...
class DeployMonitoringAgentApprovalMethod(StrEnum):
...
class DeployMonitoringAgentArchitecture(StrEnum):
...
class DeployMonitoringAgentServicePolicy(StrEnum):
    """Required node-exporter service state after package installation."""

    DISABLED_INACTIVE = "disabled-inactive"


class DeployMonitoringAgentListenPolicy(StrEnum):
    """Required pre-configuration listen policy."""

    NOT_STARTED = "not-started"


class DeployMonitoringAgentAuthorizationArtifactState(StrEnum):
...
class DeployMonitoringAgentAuthorizationProof:
    """Already-normalized ordinary approval without caller-owned scope."""

    approval_method: DeployMonitoringAgentApprovalMethod | None = None
    approved: bool = False
    ...
    def __post_init__(self) -> None:
        if self.approval_method is not None and not isinstance(
            self.approval_method, DeployMonitoringAgentApprovalMethod
        ):
            raise StateConflictError(
                "deploy monitoring-agent approval method is invalid"
            )
        if not all(
            ...
            raise StateConflictError(
                "deploy monitoring-agent authorization proof is malformed"
            )


@dataclass(frozen=True, slots=True)
class DeployMonitoringAgentProofDecision:
    """Persisted ordinary proof bound to the exact complete scope."""

    approval_method: DeployMonitoringAgentApprovalMethod
    approved: bool
    ...
    def __post_init__(self) -> None:
        if (
            self.schema_version
            != ANSIBLE_DEPLOY_MONITORING_AGENT_AUTHORIZATION_PROOF_SCHEMA_VERSION
            or not isinstance(self.approval_method, DeployMonitoringAgentApprovalMethod)
            ...
            raise StatePersistenceError(
                "deploy monitoring-agent authorization proof state is invalid"
            )
        validate_digest(
            self.proof_digest, "deploy monitoring-agent authorization proof digest"
        )
    ...
    ) -> DeployMonitoringAgentProofDecision:
        require_exact_keys(
            value,
            set(cls.__dataclass_fields__),
            "deploy monitoring-agent authorization proof",
        )
        try:
            method = DeployMonitoringAgentApprovalMethod(
                require_string(value, "approval_method")
            )
        except ValueError as error:
            raise StatePersistenceError(
                "deploy monitoring-agent authorization proof method is invalid"
            ) from error
        return cls(
...
@dataclass(frozen=True, slots=True)
class DeployMonitoringAgentPackageProvenance:
    """URL- and key-material-free identity of the node-exporter package policy."""
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise StatePersistenceError(
                "deploy monitoring-agent package provenance conflicts"
            )
        for digest in _digest_fields(self):
            validate_digest(digest, "deploy monitoring-agent package provenance digest")
    ...
    def from_object(
        cls, value: Mapping[str, object]
    ) -> DeployMonitoringAgentPackageProvenance:
        require_exact_keys(
            value,
            set(cls.__dataclass_fields__),
            "deploy monitoring-agent package provenance",
        )
        return cls(
...
@dataclass(frozen=True, slots=True)
class DeployMonitoringAgentAuthorizationScope:
    """One exact redacted node-exporter package-install intent."""
    ...
    target_digest: str
    architecture: DeployMonitoringAgentArchitecture
    image_policy_digest: str
    service_policy: DeployMonitoringAgentServicePolicy
    listen_policy: DeployMonitoringAgentListenPolicy
    configuration_permitted: bool
    process_exporter_permitted: bool
    stack_install_permitted: bool
    target_generation_permitted: bool
    ...
    def __post_init__(self) -> None:
        ...
        prohibited = (
            ...
            self.process_exporter_permitted,
            self.stack_install_permitted,
            self.target_generation_permitted,
        ...
        if (
            ...
            or not isinstance(self.architecture, DeployMonitoringAgentArchitecture)
            or self.service_policy
            is not DeployMonitoringAgentServicePolicy.DISABLED_INACTIVE
            or self.listen_policy is not DeployMonitoringAgentListenPolicy.NOT_STARTED
            or any(prohibited)
            or self.install_intent_digest != _scope_intent_digest(self)
        ):
            raise StatePersistenceError(
                "deploy monitoring-agent authorization scope policy is invalid"
            )
        for digest in _digest_fields(self):
            validate_digest(
                digest, "deploy monitoring-agent authorization scope digest"
            )
    ...
    def from_object(
        ...
        require_exact_keys(
            value,
            set(cls.__dataclass_fields__),
            "deploy monitoring-agent authorization scope",
        )
        boolean_fields = {
            "configuration_permitted",
            "process_exporter_permitted",
            "stack_install_permitted",
            "target_generation_permitted",
        ...
        try:
            for name in cls.__dataclass_fields__:
                ...
                if name in integer_fields:
                    ...
                    parsed[name] = DeployMonitoringAgentArchitecture(
                        require_string(value, name)
                    )
                elif name == "service_policy":
                    parsed[name] = DeployMonitoringAgentServicePolicy(
                        require_string(value, name)
                    )
                elif name == "listen_policy":
                    parsed[name] = DeployMonitoringAgentListenPolicy(
                        require_string(value, name)
                    )
            ...
            raise StatePersistenceError(
                "deploy monitoring-agent authorization scope enum is invalid"
            ) from error
        return cls(**parsed)  # type: ignore[arg-type]


@dataclass(frozen=True, slots=True)
class DeployMonitoringAgentAuthorization:
    ...
    package_provenance: DeployMonitoringAgentPackageProvenance
    proof: DeployMonitoringAgentProofDecision
    ...
    post_manager_agent_schema_version: str = (
        ANSIBLE_DEPLOY_POST_MANAGER_AGENT_RECONCILIATION_SCHEMA_VERSION
    )
    ...
    schema_version: str = ANSIBLE_DEPLOY_MONITORING_AGENT_AUTHORIZATION_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        if (
            ...
            != ANSIBLE_DEPLOY_MONITORING_AGENT_AUTHORIZATION_SCHEMA_VERSION
            or self.post_manager_agent_schema_version
            != ANSIBLE_DEPLOY_POST_MANAGER_AGENT_RECONCILIATION_SCHEMA_VERSION
            ...
            raise StatePersistenceError(
                "deploy monitoring-agent authorization identity or state is invalid"
            )
        ...
        for count in (
            ...
            _positive_integer(
                count, "deploy monitoring-agent authorization generation or count"
            )
        for digest in _digest_fields(self):
            validate_digest(
                digest, "deploy monitoring-agent authorization binding digest"
            )
        if self.proof.proof_digest != _proof_digest(self, self.proof.to_object()):
            raise StatePersistenceError(
                "deploy monitoring-agent authorization proof digest conflicts"
            )
        if self.authorization_digest != _authorization_digest(self):
            raise StatePersistenceError(
                "deploy monitoring-agent authorization digest conflicts"
            )
    ...
    def from_object(
        cls, value: Mapping[str, object]
    ) -> DeployMonitoringAgentAuthorization:
        require_exact_keys(
            value,
            set(cls.__dataclass_fields__),
            "deploy monitoring-agent authorization",
        )
        integer_fields = {
            "generation",
            "journal_generation",
        ...
        try:
            for name in cls.__dataclass_fields__:
                ...
                if name in integer_fields:
                    ...
                    parsed[name] = tuple(
                        DeployMonitoringAgentAuthorizationScope.from_object(
                            _mapping(scope, "deploy monitoring-agent scope")
                        )
                        for scope in _array(item, "deploy monitoring-agent scopes")
                    )
                elif name == "target_stable_ids":
                    parsed[name] = _string_tuple(item, "target stable IDs")
                elif name == "package_provenance":
                    parsed[name] = DeployMonitoringAgentPackageProvenance.from_object(
                        _mapping(
                            item,
                            "deploy monitoring-agent package provenance",
                        )
                    ...
                    parsed[name] = DeployMonitoringAgentProofDecision.from_object(
                        _mapping(item, "deploy monitoring-agent proof")
                    )
                elif name == "consumed":
                    parsed[name] = _boolean(item, name)
                else:
            ...
            raise StatePersistenceError(
                "deploy monitoring-agent authorization enum is invalid"
            ) from error
        return cls(**parsed)  # type: ignore[arg-type]


@dataclass(frozen=True, slots=True)
class StoredDeployMonitoringAgentAuthorization:
    record: DeployMonitoringAgentAuthorization
    artifact_digest: str


class DeployMonitoringAgentAuthorizationStore:
    ...
    def __init__(
        ...
        *,
        ...
        self._path = deploy_monitoring_agent_authorization_path(paths, operation_id)
    ...
    def read(
        ...
        *,
        ...
        record = DeployMonitoringAgentAuthorization.from_object(value)
        if (
            ...
            raise StatePersistenceError(
                "deploy monitoring-agent authorization identity conflicts"
            )
        return StoredDeployMonitoringAgentAuthorization(record, artifact_digest)
    ...
    def write_locked(
        ...
        *,
        ...
        StoredDeployMonitoringAgentAuthorization,
        DeployMonitoringAgentAuthorizationArtifactState,
    ]:
        _assert_operation_lock(lock, self._paths)
        ...
        if record.operation_id != self._operation_id:
            raise StatePersistenceError(
                "deploy monitoring-agent authorization operation conflicts"
            )
        if self._path.exists():
            ...
            if current.record != record:
                raise StateConflictError(
                    "deploy monitoring-agent authorization is immutable; "
                    "use a new operation"
                )
            return current, DeployMonitoringAgentAuthorizationArtifactState.REUSED
        artifact_digest = self._file.write(record.to_object(), expected_digest=None)
        return (
            StoredDeployMonitoringAgentAuthorization(record, artifact_digest),
            DeployMonitoringAgentAuthorizationArtifactState.CREATED,
        )


@dataclass(frozen=True, slots=True)
class DeployMonitoringAgentAuthorizationReport:
    """Strict redacted authorization report without executable inputs."""

    operation_id: uuid.UUID
    artifact_state: DeployMonitoringAgentAuthorizationArtifactState
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise StatePersistenceError(
                "deploy monitoring-agent authorization report is invalid"
            )
        for stable_id in self.target_stable_ids:
            if not stable_id.isascii() or _LOGICAL_ID.fullmatch(stable_id) is None:
                raise StatePersistenceError(
                    "deploy monitoring-agent authorization report target is invalid"
                )
        for digest in _digest_fields(self):
    ...
    def to_object(self) -> dict[str, object]:
        return {
            ...
            "provenance": {
                ...
                "manager_agent_evidence_digest": self.manager_agent_evidence_digest,
                "post_manager_agent": {
                    "artifact_digest": self.post_manager_agent_artifact_digest,
                    "record_digest": self.post_manager_agent_record_digest,
                    "schema_version": self.post_manager_agent_schema_version,
                },
...
class _AuthorizationContext:
    ...
    post_manager: StoredDeployPostManagerAgentReconciliation


def authorize_deploy_monitoring_agent(
    *,
    state_root: Path,
    cluster_name: str,
    operation_id: uuid.UUID,
    lock: ClusterLock,
    proof: DeployMonitoringAgentAuthorizationProof,
) -> DeployMonitoringAgentAuthorizationReport:
    """Authorize the exact complete mapped node-exporter install scope."""

    if not isinstance(proof, DeployMonitoringAgentAuthorizationProof):
        raise StateConflictError(
            "deploy monitoring-agent authorization proof is malformed"
        )
    ...
    loaded = _loaded(
        context.manager.authorization_context.monitoring.monitoring.manager.manager.chain.authorization_context
    )
    ...
    store = DeployMonitoringAgentAuthorizationStore(paths, operation_id)
    ...
    if store.path.exists():
        ...
        if stored.record != expected:
            raise StateConflictError(
                "deploy monitoring-agent authorization changed; "
                "re-plan with a new operation"
            )
        state = DeployMonitoringAgentAuthorizationArtifactState.REUSED
    else:
        ...
        try:
            ...
            raise StatePersistenceError(
                "deploy monitoring-agent authorization persistence failed"
            ) from error
    return _build_report(stored, state=state)


def deploy_monitoring_agent_authorization_path(
    paths: StatePaths, operation_id: uuid.UUID
) -> Path:
    """Return the canonical operation-bound monitoring-agent authorization path."""

    _require_canonical_paths(paths)
    operation_id = _require_operation_id(operation_id)
    path = paths.operations / (
        f"{operation_id}{DEPLOY_MONITORING_AGENT_AUTHORIZATION_FILENAME_SUFFIX}"
    )
    if path.parent != paths.operations:
        raise StatePersistenceError(
            "deploy monitoring-agent authorization path is not canonical"
        )
    return path


def deploy_monitoring_agent_authorization_id_from_filename(
    name: str,
) -> uuid.UUID | None:
    if not name.endswith(DEPLOY_MONITORING_AGENT_AUTHORIZATION_FILENAME_SUFFIX):
        return None
    value = name[: -len(DEPLOY_MONITORING_AGENT_AUTHORIZATION_FILENAME_SUFFIX)]
    try:
...
def _load_authorization_context(
    ...
    *,
    ...
    loaded = _loaded(
        manager.authorization_context.monitoring.monitoring.manager.manager.chain.authorization_context
    )
    metadata = loaded.planning.base.deploy.metadata.record
    post_manager_store = DeployPostManagerAgentReconciliationStore(paths, operation_id)
    validate_state_file(post_manager_store.path, allow_missing=True)
    if not post_manager_store.path.exists():
        raise StateConflictError(
            "deploy monitoring-agent authorization requires post-manager-agent "
            "reconciliation"
        )
    post_manager = post_manager_store.read_locked(
        lock,
        expected_cluster_uuid=metadata.cluster_uuid,
        expected_cluster_name=metadata.cluster_name,
    )
    expected_steps = _build_post_manager_steps(manager)
    expected_post = _build_post_manager_record(
        manager,
        steps=expected_steps,
        created_at=post_manager.record.created_at,
    )
    if post_manager.record != expected_post:
        raise StateConflictError(
            "deploy monitoring-agent post-manager-agent reconciliation drifted"
        )
    install = manager.authorization_context.install.evidence
    post_install = manager.authorization_context.post_install
    if (
        install.record.binding.inventory_artifact_digest
        != loaded.planning.base.deploy.inventory.digest
        or install.record.binding.trust_artifact_digest
        ...
        or install.record.binding.source_digest != loaded.source.digest
        or install.record.binding.catalog_digest != loaded.catalog_digest
        or post_install.record.evidence_artifact_digest != install.artifact_digest
    ):
        raise StateConflictError(
            "deploy monitoring-agent Scylla install provenance is not current"
        )
    return _AuthorizationContext(manager, post_manager)


def _derive_package_provenance() -> DeployMonitoringAgentPackageProvenance:
    ...
    values: dict[str, object] = {
        ...
        "package_count": len(MONITORING_AGENT_PACKAGES),
        "package_set_digest": _digest_object(list(MONITORING_AGENT_PACKAGES)),
    ...
    return DeployMonitoringAgentPackageProvenance(**values)  # type: ignore[arg-type]


def _derive_authorization_scopes(
    context: _AuthorizationContext,
    package: DeployMonitoringAgentPackageProvenance,
) -> tuple[DeployMonitoringAgentAuthorizationScope, ...]:
    manager = context.manager
    authorization_context = manager.authorization_context
    loaded = _loaded(
    ...
    if len(steps) != 1:
        raise StateConflictError(
            "deploy monitoring-agent mapped authorization scope is ambiguous"
        )
    step = steps[0]
    definition = get_playbook(_PLAYBOOK)
    ...
    if (
        ...
        raise StateConflictError(
            "only the exact active mapped monitoring-agent scope may be authorized"
        )
    ...
    if (
        ...
        raise StateConflictError(
            "deploy monitoring-agent scope is not the complete current Scylla "
            "identity set"
        )
    all_inventory_ids = {
    ...
    if any(
        ...
        raise StateConflictError("deploy monitoring-agent target role conflicts")
    ...
    install_by_id = {entry.stable_id: entry for entry in install.record.entries}
    post_install = authorization_context.post_install
    if (
        set(base_by_id) != set(stable_ids)
        or set(install_by_id) != set(stable_ids)
        or post_install.record.target_set_digest != _digest_object(list(stable_ids))
        or post_install.record.installed_count != len(stable_ids)
        or post_install.record.service_safe_count != len(stable_ids)
        or post_install.record.prohibited_action_count
    ):
        raise StateConflictError(
            "deploy monitoring-agent base-os or Scylla-install scope is incomplete"
        )
    ...
    if image_filter != ImageFilter("Ubuntu", "24.04", ImageVersionMatch.EXACT):
        raise StateConflictError(
            "deploy monitoring-agent Scylla image policy is not exact Ubuntu 24.04"
        )
    readiness = _reconstructed_readiness(planning.base)
    result: list[DeployMonitoringAgentAuthorizationScope] = []
    for index, stable_id in enumerate(stable_ids, start=1):
        ...
        values: dict[str, object] = {
            ...
            "service_policy": (DeployMonitoringAgentServicePolicy.DISABLED_INACTIVE),
            "listen_policy": DeployMonitoringAgentListenPolicy.NOT_STARTED,
            "configuration_permitted": False,
            "process_exporter_permitted": False,
            "stack_install_permitted": False,
            "target_generation_permitted": False,
        ...
        result.append(
            DeployMonitoringAgentAuthorizationScope(**values)  # type: ignore[arg-type]
        )
    return tuple(result)
...
def _validate_prerequisite_entry(
    ...
    if (
        ...
        raise StateConflictError(
            "deploy monitoring-agent Ubuntu, base-os, Scylla-install, "
            "configuration, bootstrap-health, or service gate conflicts"
        )
...
def _normalize_proof(
    ...
    *,
    ...
    if proof.approval_method is None:
        raise StateConflictError(
            "ordinary deploy monitoring-agent approval is required"
        )
    if not proof.approved:
        raise StateConflictError("ordinary deploy monitoring-agent approval was denied")
    if (
    ...
    values["proof_digest"] = _proof_digest_values(
        ...
        post_manager_agent_artifact_digest=post.artifact_digest,
        post_manager_agent_record_digest=post.record.record_digest,
        scope_digest=scope_digest,
        package_provenance_digest=package_provenance_digest,
        proof=values,
    )
    return DeployMonitoringAgentProofDecision.from_object(values)
...
def _build_authorization(
    ...
    *,
    ...
    authorization_context = context.manager.authorization_context
    loaded = _loaded(
        authorization_context.monitoring.monitoring.manager.manager.chain.authorization_context
    )
    planning = loaded.planning
    ...
    post_install = authorization_context.post_install
    base_os = authorization_context.monitoring.monitoring.manager.manager.base_os
    manager_evidence = context.manager.evidence.record
    ...
    values: dict[str, object] = {
        ...
        "final_health_evidence_digest": (
            authorization_context.monitoring.monitoring.manager.manager.bridge.record.final_health_evidence_digest
        ),
        "post_manager_agent_artifact_digest": context.post_manager.artifact_digest,
        "post_manager_agent_record_digest": post.record_digest,
    ...
    if (
        ...
        != planning.readiness.artifact_digest
        or post.evidence_artifact_digest != context.manager.evidence.artifact_digest
    ):
        raise StateConflictError(
            "deploy monitoring-agent inventory, trust, readiness, journal, "
            "install, manager-agent, or source binding drifted"
        )
    values["authorization_digest"] = _authorization_digest_from_values(values)
    return DeployMonitoringAgentAuthorization(**values)  # type: ignore[arg-type]


def _build_report(
    stored: StoredDeployMonitoringAgentAuthorization,
    *,
    state: DeployMonitoringAgentAuthorizationArtifactState,
) -> DeployMonitoringAgentAuthorizationReport:
    ...
    prohibited = tuple(
        ...
        for value in (
            ...
            scope.process_exporter_permitted,
            scope.stack_install_permitted,
            scope.target_generation_permitted,
    ...
    return DeployMonitoringAgentAuthorizationReport(
        ...
        post_manager_agent_artifact_digest=record.post_manager_agent_artifact_digest,
        post_manager_agent_record_digest=record.post_manager_agent_record_digest,
        manager_agent_evidence_digest=record.manager_agent_evidence_digest,
...
def _package_provenance_digest(
    value: DeployMonitoringAgentPackageProvenance,
) -> str:
...
def _scope_intent_digest(scope: DeployMonitoringAgentAuthorizationScope) -> str:
    ...
    return _digest_object(
        {
            ...
            "package_provenance_digest": package_provenance_digest,
            "post_manager_agent_artifact_digest": (post_manager_agent_artifact_digest),
            "post_manager_agent_record_digest": post_manager_agent_record_digest,
            "proof": proof_value,
...
def _authorization_digest(record: DeployMonitoringAgentAuthorization) -> str:
...
def _authorization_digest_from_values(values: Mapping[str, object]) -> str:
    ...
    for (
        name,
        field,
    ) in DeployMonitoringAgentAuthorization.__dataclass_fields__.items():
...
def _refuse_incompatible_or_later_artifacts(
    ...
    try:
        ...
        raise StatePersistenceError(
            "cannot safely list deploy monitoring-agent operation history"
        ) from error
    ...
    for entry in entries:
        if entry.name.startswith(prefix) and any(
            ...
            raise StateConflictError(
                "deploy monitoring-agent authorization refuses execution "
...
def _refuse_ambiguous_authorization_artifacts(
    ...
    try:
        ...
        raise StatePersistenceError(
            "cannot safely list deploy monitoring-agent authorization history"
        ) from error
    canonical = str(operation_id)
    suffix = DEPLOY_MONITORING_AGENT_AUTHORIZATION_FILENAME_SUFFIX
    for entry in entries:
        ...
        if prefix != canonical and (
            ...
            validate_state_file(entry)
            raise StateConflictError(
                "deploy monitoring-agent authorization artifacts are ambiguous"
            )
...
def _require_canonical_paths(paths: StatePaths) -> None:
    if (
        ...
        raise StatePersistenceError(
            "deploy monitoring-agent authorization paths are not canonical"
        )
...
def _assert_operation_lock(lock: ClusterLock, paths: StatePaths) -> None:
    if not isinstance(lock, ClusterLock):
        raise StateLockError(
            "deploy monitoring-agent authorization requires an acquired deploy lock"
        )
...
__all__ = [
    ...
    "ANSIBLE_DEPLOY_MONITORING_AGENT_PACKAGE_PROVENANCE_SCHEMA_VERSION",
    "DEPLOY_MONITORING_AGENT_AUTHORIZATION_FILENAME_SUFFIX",
    "DeployMonitoringAgentApprovalMethod",
    "DeployMonitoringAgentArchitecture",
    "DeployMonitoringAgentAuthorization",
    "DeployMonitoringAgentAuthorizationArtifactState",
    "DeployMonitoringAgentAuthorizationProof",
    "DeployMonitoringAgentAuthorizationReport",
    "DeployMonitoringAgentAuthorizationScope",
    "DeployMonitoringAgentAuthorizationStore",
    "DeployMonitoringAgentListenPolicy",
    "DeployMonitoringAgentPackageProvenance",
    "DeployMonitoringAgentProofDecision",
    "DeployMonitoringAgentServicePolicy",
    "StoredDeployMonitoringAgentAuthorization",
    "authorize_deploy_monitoring_agent",
    "deploy_monitoring_agent_authorization_id_from_filename",
    "deploy_monitoring_agent_authorization_path",
]
