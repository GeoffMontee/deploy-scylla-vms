"""Durable execution ownership for an authorized later Scylla join.

This internal owner derives the first unfinished sequence >=4 exclusively from
the immutable bootstrap plan, latest complete-set health, later-join safety,
and authorization chain. It records prepared and started intent before one
controlled call and persists only address-free semantic evidence. It never
advances the common journal, performs post-join health, or authorizes the next
join.
"""
...
ANSIBLE_DEPLOY_SCYLLA_LATER_JOIN_EXECUTION_BINDING_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-scylla-later-join-execution-binding/v1"
)
ANSIBLE_DEPLOY_SCYLLA_LATER_JOIN_EXECUTION_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-scylla-later-join-execution/v1"
)
ANSIBLE_DEPLOY_SCYLLA_LATER_JOIN_EVIDENCE_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-scylla-later-join-evidence/v1"
)
ANSIBLE_DEPLOY_SCYLLA_LATER_JOIN_EXECUTION_REPORT_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-scylla-later-join-execution-report/v1"
)

DEPLOY_SCYLLA_LATER_JOIN_EXECUTION_FILENAME_SUFFIX = "-execution.json"
DEPLOY_SCYLLA_LATER_JOIN_EVIDENCE_FILENAME_SUFFIX = "-evidence.json"
...
_STAGE = "later-join-existing-execution"
_SCOPE_KIND = "authorized-reconciled-later-join"
_MINIMUM_SEQUENCE = 4
...
DeployScyllaLaterJoinExecutionState = DeployScyllaJoinExecutionState
DeployScyllaLaterJoinArtifactState = DeployScyllaJoinArtifactState


@dataclass(frozen=True, slots=True)
class DeployScyllaLaterJoinExecutionBinding:
    """Value-free binding for the one exact later-join execution."""
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or self.survivor_count != self.sequence - 1
            ...
            != _digest_object(
                {
                    ...
                    "target_plan_step_digest": self.plan_step_digest,
                }
            )
            ...
            raise StatePersistenceError(
                "deploy Scylla later join execution binding conflicts"
            )
        ...
        for generation in (
            ...
            _positive_integer(generation, "later join binding generation")
        for digest in _digest_fields(self):
            if digest is None:
                raise StatePersistenceError(
                    "deploy Scylla later join binding digest is missing"
                )
            validate_digest(digest, "later join binding digest")
    ...
    def from_object(
    ...
    ) -> DeployScyllaLaterJoinExecutionBinding:
        parsed = _parse_dataclass(
            ...
            label="later join execution binding",
        )
        return cls(**parsed)  # type: ignore[arg-type]


@dataclass(frozen=True, slots=True)
class DeployScyllaLaterJoinExecution:
    """Generation-guarded at-most-once later-join execution state."""

    generation: int
    created_at: str
    updated_at: str
    binding: DeployScyllaLaterJoinExecutionBinding
    ...
    schema_version: str = ANSIBLE_DEPLOY_SCYLLA_LATER_JOIN_EXECUTION_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise StatePersistenceError("deploy Scylla later join execution conflicts")
        ...
        if (
            ...
            raise StatePersistenceError(
                "deploy Scylla later join execution timestamps conflict"
            )
        consumed = (
        ...
        if self.state is DeployScyllaLaterJoinExecutionState.PREPARED:
            valid = (
        ...
        elif self.state is DeployScyllaLaterJoinExecutionState.STARTED:
            valid = (
        ...
        elif self.state is DeployScyllaLaterJoinExecutionState.SUCCEEDED:
            valid = (
                self.invocation_count == 1
                and self.invocation_may_have_occurred
                and consumed
        ...
        elif self.state is DeployScyllaLaterJoinExecutionState.FAILED:
            valid = (
                self.invocation_count == 1
                and self.invocation_may_have_occurred
                and consumed
        ...
        if not valid:
            raise StatePersistenceError(
                "deploy Scylla later join execution state conflicts"
            )
        for value in (self.result_digest, self.evidence_digest):
            if value is not None:
                validate_digest(value, "later join execution digest")

    def to_object(self) -> dict[str, object]:
        return _dataclass_object(self, nested_fields={"binding"})

    @classmethod
    def from_object(cls, value: Mapping[str, object]) -> DeployScyllaLaterJoinExecution:
        parsed = _parse_dataclass(
            cls,
            value,
            ...
            label="later join execution",
        )
        parsed["binding"] = DeployScyllaLaterJoinExecutionBinding.from_object(
            _mapping(value["binding"], "later join binding")
        )
        ...
        try:
            ...
            raise StatePersistenceError(
                "deploy Scylla later join mutation boundary is invalid"
            ) from error
        return cls(**parsed)  # type: ignore[arg-type]


@dataclass(frozen=True, slots=True)
class DeployScyllaLaterJoinEvidence:
    """Immutable address-free semantic evidence for later join."""
    ...
    schema_version: str = ANSIBLE_DEPLOY_SCYLLA_LATER_JOIN_EVIDENCE_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            or self.sequence < _MINIMUM_SEQUENCE
            or self.sequence != self.binding.sequence
            or self.mode is not ScyllaBootstrapMode.JOIN_EXISTING
            or self.status is ScyllaBootstrapStatus.NOT_PREDICTED
            or _LOGICAL_ID.fullmatch(self.stable_id) is None
            or _digest_object(self.stable_id) != self.binding.target_digest
            or self.survivor_count != self.sequence - 1
            or self.active_seed_count != 1
            ...
            raise StatePersistenceError(
                "deploy Scylla later join semantic evidence conflicts"
            )
        if success:
            valid = (
        ...
        if not valid:
            raise StatePersistenceError(
                "deploy Scylla later join result semantics conflict"
            )
        if _optional_timestamp(self.created_at) is None:
            raise StatePersistenceError(
                "deploy Scylla later join evidence timestamp is missing"
            )
        for value in _digest_fields(self):
            if value is not None:
                validate_digest(value, "later join evidence digest")

    def to_object(self) -> dict[str, object]:
        return _dataclass_object(self, nested_fields={"binding"})

    @classmethod
    def from_object(cls, value: Mapping[str, object]) -> DeployScyllaLaterJoinEvidence:
        parsed = _parse_dataclass(
            cls,
            value,
            integer_fields={
            ...
            skip_fields={"binding"},
            label="later join evidence",
        )
        parsed["binding"] = DeployScyllaLaterJoinExecutionBinding.from_object(
            _mapping(value["binding"], "later join evidence binding")
        )
        return cls(**parsed)  # type: ignore[arg-type]


@dataclass(frozen=True, slots=True)
class StoredDeployScyllaLaterJoinExecution:
    record: DeployScyllaLaterJoinExecution
    artifact_digest: str


@dataclass(frozen=True, slots=True)
class StoredDeployScyllaLaterJoinEvidence:
    record: DeployScyllaLaterJoinEvidence
    artifact_digest: str


class DeployScyllaLaterJoinExecutionStore:
    def __init__(
        self,
        paths: StatePaths,
        operation_id: uuid.UUID,
        ...
        *,
        ...
        self._path = deploy_scylla_later_join_execution_path(
    ...
    ) -> StoredDeployScyllaLaterJoinExecution:
        value, artifact_digest = self._file.read()
        record = DeployScyllaLaterJoinExecution.from_object(value)
        if (
            record.binding.operation_id != self._operation_id
            or record.binding.sequence != self._sequence
            or record.binding.cluster_uuid != expected_cluster_uuid
            or record.binding.cluster_name != expected_cluster_name
        ):
            raise StatePersistenceError(
                "deploy Scylla later join execution identity conflicts"
            )
        return StoredDeployScyllaLaterJoinExecution(record, artifact_digest)
    ...
    def write_locked(
        ...
        *,
        ...
        if record.binding.operation_id != self._operation_id:
            raise StatePersistenceError(
                "deploy Scylla later join execution operation conflicts"
            )
        if self._path.exists():
            ...
            if (
                ...
                raise StatePersistenceError(
                    "deploy Scylla later join execution changed concurrently"
                )
            ...
            or record.state is not DeployScyllaLaterJoinExecutionState.PREPARED
        ):
            raise StatePersistenceError(
                "initial deploy Scylla later join execution generation conflicts"
            )
...
class DeployScyllaLaterJoinEvidenceStore:
    def __init__(
        self,
        paths: StatePaths,
        operation_id: uuid.UUID,
        ...
        *,
        ...
        self._operation_id = _require_operation_id(operation_id)
        self._sequence = _require_later_sequence(sequence)
        self._path = deploy_scylla_later_join_evidence_path(
            paths, operation_id, sequence
        )
        self._file = AtomicJsonFile(
            self._path, replace=replace_file, token_factory=token_factory
    ...
    def read(
        ...
        *,
        expected_cluster_uuid: uuid.UUID,
        expected_cluster_name: str,
    ) -> StoredDeployScyllaLaterJoinEvidence:
        value, artifact_digest = self._file.read()
        record = DeployScyllaLaterJoinEvidence.from_object(value)
        if (
            record.binding.operation_id != self._operation_id
            or record.binding.sequence != self._sequence
            or record.binding.cluster_uuid != expected_cluster_uuid
            ...
            raise StatePersistenceError(
                "deploy Scylla later join evidence identity conflicts"
            )
        return StoredDeployScyllaLaterJoinEvidence(record, artifact_digest)
        ...
        if self._path.exists():
            ...
            if current.record != record:
                raise StateConflictError(
                    "deploy Scylla later join evidence is immutable"
                )
            return current, DeployScyllaLaterJoinArtifactState.REUSED
        artifact_digest = self._file.write(record.to_object(), expected_digest=None)
        return (
            StoredDeployScyllaLaterJoinEvidence(record, artifact_digest),
            DeployScyllaLaterJoinArtifactState.CREATED,
        )


@dataclass(frozen=True, slots=True)
class DeployScyllaLaterJoinExecutionReport:
    operation_id: uuid.UUID
    sequence: int
    execution_state: DeployScyllaLaterJoinExecutionState
    execution_artifact_state: DeployScyllaLaterJoinArtifactState
    evidence_artifact_state: DeployScyllaLaterJoinArtifactState
    ...
    later_join_state: str = "waiting-for-fresh-complete-set-health"
    ...
    def __post_init__(self) -> None:
        ...
        if (
            ...
            != ANSIBLE_DEPLOY_SCYLLA_LATER_JOIN_EVIDENCE_SCHEMA_VERSION
            or self.execution_state is not DeployScyllaLaterJoinExecutionState.SUCCEEDED
            ...
            or self.later_join_state != "waiting-for-fresh-complete-set-health"
            or self.post_join_health_state != "not-performed"
            or self.public_workflow_state != "unavailable"
        ):
            raise StatePersistenceError(
                "deploy Scylla later join execution report conflicts"
            )
        for value in _digest_fields(self):
            if value is None:
                raise StatePersistenceError(
                    "deploy Scylla later join report digest is missing"
                )
            validate_digest(value, "later join report digest")
...
class _ExecutionContext:
    authorization: StoredDeployScyllaLaterJoinAuthorization
    binding: DeployScyllaLaterJoinExecutionBinding
...
def execute_deploy_scylla_later_join(
    *,
    state_root: Path,
    cluster_name: str,
    operation_id: uuid.UUID,
...
) -> DeployScyllaLaterJoinExecutionReport:
    """Execute only the canonical authorized later join."""
    ...
    _refuse_ambiguous_or_later_artifacts(paths, operation_id, sequence)
    execution_store = DeployScyllaLaterJoinExecutionStore(paths, operation_id, sequence)
    evidence_store = DeployScyllaLaterJoinEvidenceStore(paths, operation_id, sequence)
    ...
    if (
        ...
        and execution.record.state is DeployScyllaLaterJoinExecutionState.SUCCEEDED
    ):
        if evidence is None:
            raise StateConflictError(
                "completed deploy Scylla later join evidence is unavailable"
            )
        return _build_report(
            context,
            execution,
            evidence,
            execution_state=DeployScyllaLaterJoinArtifactState.REUSED,
            evidence_state=DeployScyllaLaterJoinArtifactState.REUSED,
        )
    if (
        execution is not None
        and execution.record.state is not DeployScyllaLaterJoinExecutionState.PREPARED
    ):
        raise StateConflictError(
            "deploy Scylla later join execution requires manual recovery "
            "and cannot retry"
        )

    service = AnsibleService(builder, runner)
    if service.version(lock) != toolchain:
        raise StateConflictError("deploy Scylla later join toolchain drifted")
    ...
    if before_prepared.binding != context.binding:
        raise StateConflictError(
            "deploy Scylla later join state drifted before prepared intent"
        )
    ...
    if execution is None:
        try:
            ...
            raise StatePersistenceError(
                "deploy Scylla later join prepared intent persistence "
    ...
    if before_start.binding != context.binding:
        raise StateConflictError("deploy Scylla later join state drifted before start")
    _validate_prefix(before_start, execution, evidence)
    try:
        execution = _persist_started(execution_store, execution, lock=lock)
    except StatePersistenceError as error:
        raise StatePersistenceError(
            "deploy Scylla later join authorization consumption "
            "failed before invocation"
        ) from error
    ...
    try:
        ...
        if command_digest != before_start.scope.command_digest:
            raise AnsibleResultError(
                "deploy Scylla later join command result identity conflicts"
            )
    except KeyboardInterrupt:
        _persist_uncertain_or_raise(
            execution_store,
            execution,
            DeployScyllaLaterJoinExecutionState.INTERRUPTED,
            lock=lock,
        )
        raise AnsibleError(
            "deploy Scylla later join was interrupted; manual recovery required"
        ) from None
        ...
        _persist_uncertain_or_raise(
            ...
            (
                ...
                else DeployScyllaLaterJoinExecutionState.MALFORMED_RESULT
            ),
            lock=lock,
        )
        raise AnsibleError(
            "deploy Scylla later join execution is uncertain; manual recovery required"
        ) from error
    ...
    try:
        ...
        if after.binding != context.binding:
            raise StateConflictError(
                "deploy Scylla later join state changed after invocation"
            )
    except (StateConflictError, StatePersistenceError) as error:
        _persist_uncertain_or_raise(
            execution_store,
            execution,
            DeployScyllaLaterJoinExecutionState.MALFORMED_RESULT,
            lock=lock,
        )
        raise StateConflictError(
            "deploy Scylla later join state changed after invocation; "
    ...
    try:
        ...
        raise AnsibleError(
            "deploy Scylla later join result is not strict semantic "
    ...
    try:
        ...
        raise StatePersistenceError(
            "deploy Scylla later join evidence persistence failed; "
            "manual recovery required"
        ) from error
    terminal_state = (
        DeployScyllaLaterJoinExecutionState.SUCCEEDED
        if evidence_record.status is ScyllaBootstrapStatus.BOOTSTRAPPED
        else DeployScyllaLaterJoinExecutionState.FAILED
    )
    try:
        ...
        raise StatePersistenceError(
            "deploy Scylla later join terminal persistence failed; "
            "manual recovery required"
        ) from error
    if terminal_state is DeployScyllaLaterJoinExecutionState.FAILED:
        raise AnsibleError(
            "deploy Scylla later join preserved strict failure evidence; "
    ...
    return _build_report(
        ...
        execution,
        evidence,
        execution_state=DeployScyllaLaterJoinArtifactState.UPDATED,
        evidence_state=evidence_state,
    )


def deploy_scylla_later_join_execution_path(
    ...
    return _artifact_path(
        ...
        operation_id,
        sequence,
        DEPLOY_SCYLLA_LATER_JOIN_EXECUTION_FILENAME_SUFFIX,
    )


def deploy_scylla_later_join_evidence_path(
    paths: StatePaths, operation_id: uuid.UUID, sequence: int
) -> Path:
    return _artifact_path(
        paths,
        operation_id,
        sequence,
        DEPLOY_SCYLLA_LATER_JOIN_EVIDENCE_FILENAME_SUFFIX,
    )


def deploy_scylla_later_join_execution_id_from_filename(
    name: str,
) -> tuple[uuid.UUID, int] | None:
    return _operation_id_from_filename(
        name, DEPLOY_SCYLLA_LATER_JOIN_EXECUTION_FILENAME_SUFFIX
    )


def deploy_scylla_later_join_evidence_id_from_filename(
    name: str,
) -> tuple[uuid.UUID, int] | None:
    return _operation_id_from_filename(
        name, DEPLOY_SCYLLA_LATER_JOIN_EVIDENCE_FILENAME_SUFFIX
    )
...
def _load_execution_context(
    ...
    *,
    ...
    scope = _derive_later_join_scope(authorized)
    chain = authorized.chain
    sequence = scope.sequence
    if expected_sequence is not None and sequence != expected_sequence:
        raise StateConflictError(
            "deploy Scylla later join sequence does not match canonical history"
        )
    ...
    identity = chain.latest_health_evidence.record.binding
    if (
        ...
        or scope.completed_prefix_count != sequence - 1
        or scope.completed_prefix_count != chain.completed_prefix_count
    ...
    if (
        ...
        or authorization.record.scope.sequence != sequence
        or authorization.record.scope.mode is not ScyllaBootstrapMode.JOIN_EXISTING
    ):
        raise StateConflictError(
            "deploy Scylla later join authorization is stale or consumed"
        )
    ...
    if (
        ...
        raise StateConflictError(
            "deploy Scylla later join catalog or source policy conflicts"
        )
    ...
    if len(matching) != 1:
        raise StateConflictError("deploy Scylla later join target is ambiguous")
    ...
    if not isinstance(host.scylla_datacenter, str) or not isinstance(
        ...
        raise StateConflictError("deploy Scylla later join topology is incomplete")
    survivor_ids = chain.survivor_ids
    if (
        survivor_ids != tuple(sorted(set(survivor_ids)))
        or len(survivor_ids) != sequence - 1
        or len(survivor_ids) != scope.completed_prefix_count
        or _digest_object(list(survivor_ids)) != scope.survivor_set_digest
        or stable_id in survivor_ids
    ):
        raise StateConflictError(
            "deploy Scylla later join survivor membership conflicts"
        )
    ...
    if (
        ...
        raise StateConflictError(
            "deploy Scylla later join prerequisite membership conflicts"
        )
    intent = intents[0]
    ...
    plan_step = chain.plan_steps[sequence - 1]
    preceding_step = chain.plan_steps[sequence - 2]
    safety_step = authorized.safety_reconciliation.record.steps[sequence - 1]
    health_step = chain.latest_health_reconciliation.record.steps[sequence - 1]
    ...
    if (
        ...
        raise StateConflictError(
            "deploy Scylla later join target, health, topology, seed, "
    ...
    if (
        ...
        raise StateConflictError(
            "deploy Scylla later join anchored command policy conflicts"
        )
    ...
    later = authorized.safety_reconciliation.record.steps[sequence:]
    if any(
        item.status is not DeployScyllaLaterJoinSafetyStepStatus.WAITING
        or item.authorization_state != "waiting"
        or item.blockers != ("preceding-join-not-completed",)
        for item in later
    ):
        raise StateConflictError(
            "deploy Scylla later join later sequence scope advanced"
        )
    ...
    base_chain = chain.base_chain
    bootstrap_context = base_chain.bootstrap_context_artifact_digest
    binding_values: dict[str, object] = {
        ...
        "bootstrap_context_record_digest": base_chain.bootstrap_context_record_digest,
        "bootstrap_plan_artifact_digest": base_chain.bootstrap_plan_artifact_digest,
        "bootstrap_plan_digest": base_chain.bootstrap_plan_digest,
        "first_join_authorization_artifact_digest": (
            base_chain.first_join_authorization.artifact_digest
        ),
        "first_join_authorization_digest": (
            base_chain.first_join_authorization.record.authorization_digest
        ),
        "first_join_execution_artifact_digest": (
            base_chain.first_join_execution.artifact_digest
        ),
        "first_join_execution_binding_digest": (
            base_chain.first_join_execution.record.binding.binding_digest
        ),
        "first_join_evidence_artifact_digest": (
            base_chain.first_join_evidence.artifact_digest
        ),
        "first_join_evidence_digest": (
            base_chain.first_join_evidence.record.evidence_digest
        ),
        ...
        "completed_prefix_digest": scope.completed_prefix_digest,
        "order_digest": scope.order_digest,
        "target_digest": scope.target_digest,
        "plan_step_digest": scope.bootstrap_plan_step_digest,
        "preceding_step_digest": plan_step.preceding_step_digest,
        "latest_health_step_digest": scope.latest_health_step_digest,
...
def _semantic_evidence(
    ...
    if (
        ...
        raise AnsibleResultError(
            "deploy Scylla later join strict result identity conflicts"
        )
    try:
        ...
        raise AnsibleResultError(
            "deploy Scylla later join strict result is malformed"
        ) from error
    ...
    return DeployScyllaLaterJoinEvidence(**values)  # type: ignore[arg-type]
...
def _validate_semantic_result(
    ...
    if (
        ...
        raise AnsibleResultError("deploy Scylla later join result scope conflicts")
    if evidence.status is ScyllaBootstrapStatus.BOOTSTRAPPED:
        if (
            ...
            raise AnsibleResultError(
                "deploy Scylla later join success evidence conflicts"
            )
        return
    if evidence.status is not ScyllaBootstrapStatus.FAILED:
        raise AnsibleResultError(
            "deploy Scylla later join check-mode result is forbidden"
        )
    ...
    if not valid:
        raise AnsibleResultError(
            "deploy Scylla later join failure recovery evidence conflicts"
        )


def _read_execution(
    store: DeployScyllaLaterJoinExecutionStore,
    context: _ExecutionContext,
    lock: ClusterLock,
) -> StoredDeployScyllaLaterJoinExecution | None:
    return (
        store.read_locked(
            lock,
...
def _read_evidence(
    store: DeployScyllaLaterJoinEvidenceStore,
    context: _ExecutionContext,
    lock: ClusterLock,
) -> StoredDeployScyllaLaterJoinEvidence | None:
    return (
        store.read_locked(
            lock,
            expected_cluster_uuid=context.binding.cluster_uuid,
...
def _validate_prefix(
    context: _ExecutionContext,
    execution: StoredDeployScyllaLaterJoinExecution | None,
    evidence: StoredDeployScyllaLaterJoinEvidence | None,
) -> None:
    if execution is None:
        if evidence is not None:
            raise StateConflictError(
                "deploy Scylla later join evidence exists without execution"
            )
        return
    ...
    if (
        ...
        raise StateConflictError(
            "deploy Scylla later join execution provenance is stale"
        )
    if evidence is None:
        if record.state in {
            DeployScyllaLaterJoinExecutionState.SUCCEEDED,
            DeployScyllaLaterJoinExecutionState.FAILED,
        }:
            raise StateConflictError(
                "deploy Scylla later join terminal evidence is missing"
            )
        return
    if (
        ...
        or record.state
        not in {
            DeployScyllaLaterJoinExecutionState.STARTED,
            DeployScyllaLaterJoinExecutionState.SUCCEEDED,
            DeployScyllaLaterJoinExecutionState.FAILED,
        }
    ):
        raise StateConflictError(
            "deploy Scylla later join execution and evidence conflict"
        )
    ...
    *,
    ...
    return store.write_locked(
        DeployScyllaLaterJoinExecution(
            ...
            updated_at=now,
            binding=context.binding,
            state=DeployScyllaLaterJoinExecutionState.PREPARED,
            stable_id=context.scope.stable_id,
...
def _persist_started(
    store: DeployScyllaLaterJoinExecutionStore,
    current: StoredDeployScyllaLaterJoinExecution,
    *,
    lock: ClusterLock,
) -> StoredDeployScyllaLaterJoinExecution:
    if current.record.state is not DeployScyllaLaterJoinExecutionState.PREPARED:
        raise StateConflictError(
            "deploy Scylla later join start requires prepared intent"
        )
    now = _timestamp()
    return store.write_locked(
        replace(
            ...
            updated_at=now,
            state=DeployScyllaLaterJoinExecutionState.STARTED,
            started_at=now,
...
def _persist_uncertain_or_raise(
    ...
    *,
    ...
    try:
        ...
        raise StatePersistenceError(
            "deploy Scylla later join uncertain outcome persistence failed; "
            "manual recovery required"
        ) from error
...
def _persist_uncertain(
    ...
    *,
    ...
    if (
        current.record.state is not DeployScyllaLaterJoinExecutionState.STARTED
        or state
        not in {
            DeployScyllaLaterJoinExecutionState.FAILED,
            DeployScyllaLaterJoinExecutionState.TIMED_OUT,
            DeployScyllaLaterJoinExecutionState.INTERRUPTED,
            DeployScyllaLaterJoinExecutionState.UNREACHABLE,
            DeployScyllaLaterJoinExecutionState.MALFORMED_RESULT,
        }
    ):
        raise StatePersistenceError(
            "deploy Scylla later join uncertain transition conflicts"
        )
    now = _timestamp()
    return store.write_locked(
        replace(
...
def _persist_terminal(
    ...
    current: StoredDeployScyllaLaterJoinExecution,
    *,
    evidence: DeployScyllaLaterJoinEvidence,
    exit_code: int,
    state: DeployScyllaLaterJoinExecutionState,
    lock: ClusterLock,
) -> StoredDeployScyllaLaterJoinExecution:
    if (
        ...
        or (state is DeployScyllaLaterJoinExecutionState.SUCCEEDED)
        != (evidence.status is ScyllaBootstrapStatus.BOOTSTRAPPED)
    ):
        raise StateConflictError(
            "deploy Scylla later join terminal transition conflicts"
        )
    now = _timestamp()
    return store.write_locked(
        replace(
...
def _build_report(
    context: _ExecutionContext,
    execution: StoredDeployScyllaLaterJoinExecution,
    evidence: StoredDeployScyllaLaterJoinEvidence,
    *,
    execution_state: DeployScyllaLaterJoinArtifactState,
    evidence_state: DeployScyllaLaterJoinArtifactState,
) -> DeployScyllaLaterJoinExecutionReport:
    record = evidence.record
    if (
        execution.record.state is not DeployScyllaLaterJoinExecutionState.SUCCEEDED
        or not execution.record.completed
        or record.status is not ScyllaBootstrapStatus.BOOTSTRAPPED
    ):
        raise StateConflictError("deploy Scylla later join execution is not successful")
    return DeployScyllaLaterJoinExecutionReport(
...
def _validate_execution_transition(
    current: DeployScyllaLaterJoinExecution,
    replacement: DeployScyllaLaterJoinExecution,
) -> None:
    if (
        ...
        raise StatePersistenceError(
            "deploy Scylla later join execution transition is invalid"
        )
    if current.state is DeployScyllaLaterJoinExecutionState.PREPARED:
        valid = replacement.state is DeployScyllaLaterJoinExecutionState.STARTED
    elif current.state is DeployScyllaLaterJoinExecutionState.STARTED:
        valid = replacement.state not in {
            DeployScyllaLaterJoinExecutionState.PREPARED,
            DeployScyllaLaterJoinExecutionState.STARTED,
        }
    else:
        valid = False
    if not valid:
        raise StatePersistenceError(
            "deploy Scylla later join execution transition conflicts"
        )


def _binding_digest(record: DeployScyllaLaterJoinExecutionBinding) -> str:
...
def _binding_digest_from_values(values: Mapping[str, object]) -> str:
    ...
    for (
        name,
        field,
    ) in DeployScyllaLaterJoinExecutionBinding.__dataclass_fields__.items():
        value.setdefault(name, _json_value(field.default))
    value["binding_digest"] = ""
    return _digest_object(value)


def _evidence_digest(record: DeployScyllaLaterJoinEvidence) -> str:
...
def _evidence_digest_from_values(values: Mapping[str, object]) -> str:
    ...
    for (
        name,
        field,
    ) in DeployScyllaLaterJoinEvidence.__dataclass_fields__.items():
        value.setdefault(name, _json_value(field.default))
...
def _artifact_path(
    ...
    if path.parent != paths.operations:
        raise StatePersistenceError(
            "deploy Scylla later join artifact path is not canonical"
        )
    return path
...
def _refuse_ambiguous_or_later_artifacts(
    ...
    try:
        ...
        raise StatePersistenceError(
            "cannot safely list deploy Scylla later join artifacts"
        ) from error
    suffixes = (
        DEPLOY_SCYLLA_LATER_JOIN_EXECUTION_FILENAME_SUFFIX,
        DEPLOY_SCYLLA_LATER_JOIN_EVIDENCE_FILENAME_SUFFIX,
    ...
    later_fragments = (
        ".ansible-deploy-scylla-later-join-health",
        ".ansible-deploy-scylla-post-later-join-sequence-",
        ".ansible-deploy-post-scylla-later-join",
    ...
    for entry in entries:
        if entry.name.startswith(str(operation_id)) and any(
            fragment in entry.name for fragment in later_fragments
        ):
            artifact_sequence = _later_artifact_sequence(entry.name)
            if artifact_sequence is None or artifact_sequence >= sequence:
                validate_state_file(entry)
                raise StateConflictError(
                    "deploy Scylla later join execution refuses later membership history"
                )
        for suffix in suffixes:
            ...
            if (
                ...
                and "-safety-" not in entry.name
            ):
                validate_state_file(entry)
                raise StateConflictError(
                    "deploy Scylla later join artifacts are ambiguous"
                )
...
def _require_canonical_paths(paths: StatePaths) -> None:
    if (
        ...
        raise StatePersistenceError("deploy Scylla later join paths are not canonical")


def _assert_operation_lock(lock: ClusterLock, paths: StatePaths) -> None:
    if not isinstance(lock, ClusterLock):
        raise StateLockError(
            "deploy Scylla later join execution requires an acquired deploy lock"
        )
