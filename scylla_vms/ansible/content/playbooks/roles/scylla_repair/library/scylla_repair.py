#!/usr/bin/python
"""Run one fixed direct repair and return only bounded normalized evidence."""
...
_COMMANDS = {
    ...
    "repair": ["/usr/bin/nodetool", "repair"],
...
def _inspect(
    ...
    pending_repair = not re.search(
        r"(?i)(repair|validation|anticompaction).*(pending|active|running)",
            ...
            json.dumps(
                {
                    ...
                    "streaming_complete": no_streaming,
    ...
    return {
        ...
        "pending_compactions": not pending_compactions,
        "pending_repair": not pending_repair,
...
def main() -> None:
    ...
    if (
        ...
        module.fail_json(msg="Scylla repair arguments are invalid", changed=False)
    if action == "inspect":
        module.exit_json(
            changed=False,
            inspection=_inspect(expected_ring, host_id, server_version, timeout),
        )
    rc, _, timed_out = _run(["/usr/bin/nodetool", "repair"], timeout)
    module.exit_json(
        changed=True,
        repair={
            "command": ["/usr/bin/nodetool", "repair"],
