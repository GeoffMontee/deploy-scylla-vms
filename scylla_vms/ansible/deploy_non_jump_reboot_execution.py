"""Durable serial execution for the distinct non-jump deploy reboot scope.

This internal owner derives every target and runtime value from the immutable
non-jump reboot plan. It records intent before every controlled invocation and
never retries a target once an invocation may have occurred.
"""
...
ANSIBLE_DEPLOY_NON_JUMP_REBOOT_EXECUTION_BINDING_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-non-jump-reboot-execution-binding/v1"
)
ANSIBLE_DEPLOY_NON_JUMP_REBOOT_EXECUTION_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-non-jump-reboot-execution/v1"
)
ANSIBLE_DEPLOY_NON_JUMP_REBOOT_EVIDENCE_ENTRY_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-non-jump-reboot-evidence-entry/v1"
)
ANSIBLE_DEPLOY_NON_JUMP_REBOOT_EVIDENCE_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-non-jump-reboot-evidence/v1"
)
ANSIBLE_DEPLOY_NON_JUMP_REBOOT_EXECUTION_REPORT_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-non-jump-reboot-execution-report/v1"
)

DEPLOY_NON_JUMP_REBOOT_EXECUTION_FILENAME_SUFFIX = (
    ".ansible-deploy-non-jump-reboot-execution.json"
)
DEPLOY_NON_JUMP_REBOOT_EVIDENCE_FILENAME_SUFFIX = (
...
_SCOPE_KIND = "non-jump-reboot-required-hosts"
...
class DeployNonJumpRebootExecutionState(StrEnum):
...
class DeployNonJumpRebootArtifactState(StrEnum):
    """Persistence outcome for the distinct execution companions."""
...
class DeployNonJumpRebootExecutionBinding:
    """Exact address-free provenance for the ordered non-jump reboot scope."""
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise StatePersistenceError(
                "deploy non-jump reboot execution binding is invalid"
            )
        validate_cluster_name(self.cluster_name)
        for value in _binding_digests(self):
            validate_digest(value, "deploy non-jump reboot execution binding digest")
        if self.binding_digest != _binding_digest(self):
            raise StatePersistenceError(
                "deploy non-jump reboot execution binding digest conflicts"
            )
    ...
    def from_object(
        ...
        require_exact_keys(value, set(cls.__dataclass_fields__), "non-jump binding")
        integers = {
            "journal_generation",
...
@dataclass(frozen=True, slots=True)
class DeployNonJumpRebootExecutionAttempt:
    """One durable exact-target non-jump reboot attempt."""
    ...
    source_digest: str
    request_digest: str
    state: DeployNonJumpRebootExecutionState
    prepared_at: str
    started_at: str | None
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise StatePersistenceError(
                "deploy non-jump reboot attempt identity is invalid"
            )
        ...
        for value in (
            ...
            validate_digest(value, "deploy non-jump reboot attempt digest")
        for outcome_digest in (self.result_digest, self.evidence_digest):
            if outcome_digest is not None:
                validate_digest(
                    outcome_digest, "deploy non-jump reboot attempt outcome digest"
                )
        if self.state is DeployNonJumpRebootExecutionState.PREPARED:
            valid = (
        ...
        elif self.state is DeployNonJumpRebootExecutionState.STARTED:
            valid = (
            ...
            valid = (
                ...
                and self.invocation_may_have_occurred
                and self.manual_recovery_required
                == (self.state is not DeployNonJumpRebootExecutionState.SUCCEEDED)
            )
            if self.state in {
                DeployNonJumpRebootExecutionState.SUCCEEDED,
                DeployNonJumpRebootExecutionState.FAILED,
                DeployNonJumpRebootExecutionState.UNREACHABLE,
            }:
                valid = (
                    valid
        ...
        if not valid:
            raise StatePersistenceError(
                "deploy non-jump reboot attempt state is invalid"
            )
    ...
    def from_object(
        ...
        require_exact_keys(value, set(cls.__dataclass_fields__), "non-jump attempt")
        try:
            return cls(
                ...
                state=DeployNonJumpRebootExecutionState(require_string(value, "state")),
            ...
            raise StatePersistenceError(
                "deploy non-jump reboot attempt enum is invalid"
            ) from error


@dataclass(frozen=True, slots=True)
class DeployNonJumpRebootExecution:
    """Generation-guarded serial non-jump reboot execution."""
    ...
    attempts: tuple[DeployNonJumpRebootExecutionAttempt, ...]
    schema_version: str = ANSIBLE_DEPLOY_NON_JUMP_REBOOT_EXECUTION_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        if (
            ...
            != sum(
                attempt.state is DeployNonJumpRebootExecutionState.SUCCEEDED
            ...
            != (
                self.state is DeployNonJumpRebootExecutionState.SUCCEEDED
            ...
            raise StatePersistenceError(
                "deploy non-jump reboot execution summary conflicts"
            )
        parse_timestamp(self.created_at)
        parse_timestamp(self.updated_at)
        if any(
            attempt.state is not DeployNonJumpRebootExecutionState.SUCCEEDED
            for attempt in self.attempts[:-1]
        ):
            raise StatePersistenceError(
                "deploy non-jump reboot execution skipped uncertainty"
            )
    ...
    @classmethod
    def from_object(cls, value: Mapping[str, object]) -> DeployNonJumpRebootExecution:
        require_exact_keys(value, set(cls.__dataclass_fields__), "non-jump execution")
        try:
            return cls(
                ...
                binding=DeployNonJumpRebootExecutionBinding.from_object(
                    _mapping(value["binding"], "non-jump binding")
                ),
                state=DeployNonJumpRebootExecutionState(require_string(value, "state")),
                authorization_consumed=_boolean(
                    value["authorization_consumed"], "authorization_consumed"
                ...
                attempts=tuple(
                    DeployNonJumpRebootExecutionAttempt.from_object(
                        _mapping(item, "non-jump attempt")
                    )
                    for item in _array(value["attempts"], "non-jump attempts")
                ),
                schema_version=require_string(value, "schema_version"),
            )
        except ValueError as error:
            raise StatePersistenceError(
                "deploy non-jump reboot execution enum is invalid"
            ) from error


@dataclass(frozen=True, slots=True)
class DeployNonJumpRebootEvidenceEntry:
    """One address-free strict post-reboot semantic checkpoint."""
    ...
    evidence_digest: str
    result_schema_version: str = DEPLOY_REBOOT_RESULT_SCHEMA_VERSION
    schema_version: str = ANSIBLE_DEPLOY_NON_JUMP_REBOOT_EVIDENCE_ENTRY_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        if (
            ...
            or not 0 <= self.elapsed_seconds <= 1800
        ):
            raise StatePersistenceError(
                "deploy non-jump reboot evidence entry is invalid"
            )
        for value in (
            self.target_plan_digest,
            self.variables_digest,
            ...
            validate_digest(value, "deploy non-jump reboot evidence digest")
        if self.evidence_digest != _entry_digest(self):
            raise StatePersistenceError(
                "deploy non-jump reboot evidence digest conflicts"
            )
        DeployRebootResult(
            self.stable_id,
            self.role,
    ...
    def from_object(
        ...
        require_exact_keys(value, expected, "non-jump reboot evidence entry")
        try:
            return cls(
                sequence=_integer(value["sequence"], "sequence"),
            ...
            raise StatePersistenceError(
                "deploy non-jump reboot evidence enum is invalid"
            ) from error


@dataclass(frozen=True, slots=True)
class DeployNonJumpRebootEvidence:
    """Immutable-prefix non-jump reboot semantic evidence."""

    generation: int
    created_at: str
    updated_at: str
    binding: DeployNonJumpRebootExecutionBinding
    entries: tuple[DeployNonJumpRebootEvidenceEntry, ...]
    schema_version: str = ANSIBLE_DEPLOY_NON_JUMP_REBOOT_EVIDENCE_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise StatePersistenceError(
                "deploy non-jump reboot evidence summary conflicts"
            )
        parse_timestamp(self.created_at)
        parse_timestamp(self.updated_at)
    ...
    @classmethod
    def from_object(cls, value: Mapping[str, object]) -> DeployNonJumpRebootEvidence:
        require_exact_keys(value, set(cls.__dataclass_fields__), "non-jump evidence")
        return cls(
            generation=_integer(value["generation"], "generation"),
            created_at=require_string(value, "created_at"),
            updated_at=require_string(value, "updated_at"),
            binding=DeployNonJumpRebootExecutionBinding.from_object(
                _mapping(value["binding"], "non-jump evidence binding")
            ),
            entries=tuple(
                DeployNonJumpRebootEvidenceEntry.from_object(
                    _mapping(item, "non-jump evidence entry")
                )
                for item in _array(value["entries"], "non-jump evidence entries")
            ),
            schema_version=require_string(value, "schema_version"),
        )
...
@dataclass(frozen=True, slots=True)
class StoredDeployNonJumpRebootExecution:
    record: DeployNonJumpRebootExecution
    artifact_digest: str


@dataclass(frozen=True, slots=True)
class StoredDeployNonJumpRebootEvidence:
    record: DeployNonJumpRebootEvidence
    artifact_digest: str


class DeployNonJumpRebootExecutionStore:
    ...
    def __init__(
        ...
        *,
        ...
        self._path = deploy_non_jump_reboot_execution_path(paths, operation_id)
    ...
    def read(
        ...
        record = DeployNonJumpRebootExecution.from_object(value)
        if (
            ...
            raise StatePersistenceError(
                "deploy non-jump reboot execution identity conflicts"
            )
        return StoredDeployNonJumpRebootExecution(record, artifact_digest)
    ...
    def write_locked(
        ...
        *,
        ...
        if record.binding.operation_id != self._operation_id:
            raise StatePersistenceError(
                "deploy non-jump reboot execution operation conflicts"
            )
        if self._path.exists():
            ...
            if (
                ...
                raise StateConflictError(
                    "deploy non-jump reboot execution changed concurrently"
                )
            _validate_execution_transition(current.record, record)
        elif expected_generation != 0 or expected_digest is not None:
            raise StateConflictError(
                "deploy non-jump reboot execution prefix is missing"
            )
        elif record.generation != 1:
            raise StatePersistenceError(
                "deploy non-jump reboot initial execution is invalid"
            )
...
class DeployNonJumpRebootEvidenceStore:
    ...
    def __init__(
        ...
        *,
        ...
        self._paths = paths
        self._operation_id = _require_operation_id(operation_id)
        self._path = deploy_non_jump_reboot_evidence_path(paths, operation_id)
        self._file = AtomicJsonFile(
            self._path, replace=replace_file, token_factory=token_factory
        )
    ...
    def read(
        ...
        value, artifact_digest = self._file.read()
        record = DeployNonJumpRebootEvidence.from_object(value)
        if (
            record.binding.operation_id != self._operation_id
            or record.binding.cluster_uuid != expected_cluster_uuid
            or record.binding.cluster_name != expected_cluster_name
            ...
            raise StatePersistenceError(
                "deploy non-jump reboot evidence identity conflicts"
            )
        return StoredDeployNonJumpRebootEvidence(record, artifact_digest)
    ...
    def append_locked(
        ...
        *,
        ...
        if record.binding.operation_id != self._operation_id:
            raise StatePersistenceError(
                "deploy non-jump reboot evidence operation conflicts"
            )
        if self._path.exists():
            current = self.read_locked(
                lock,
            ...
            if (
                ...
                raise StateConflictError(
                    "deploy non-jump reboot evidence changed concurrently"
                )
            if (
                ...
                raise StatePersistenceError(
                    "deploy non-jump reboot evidence transition is invalid"
                )
        elif expected_generation != 0 or expected_digest is not None:
            raise StateConflictError(
                "deploy non-jump reboot evidence prefix is missing"
            )
        elif record.generation != 1 or len(record.entries) != 1:
            raise StatePersistenceError(
                "deploy non-jump reboot initial evidence is invalid"
            )
        artifact_digest = self._file.write(
            record.to_object(), expected_digest=expected_digest
        )
...
@dataclass(frozen=True, slots=True)
class DeployNonJumpRebootExecutionReport:
    """Strict redacted report for the non-jump reboot execution owner."""

    operation_id: uuid.UUID
    execution_artifact_state: DeployNonJumpRebootArtifactState
    evidence_artifact_state: DeployNonJumpRebootArtifactState
    execution_state: str
    ...
    schema_version: str = ANSIBLE_DEPLOY_NON_JUMP_REBOOT_EXECUTION_REPORT_SCHEMA_VERSION
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise StatePersistenceError(
                "deploy non-jump reboot execution report is invalid"
            )
        counts = (
        ...
        if any(count < 0 or count > self.completed_target_count for count in counts):
            raise StatePersistenceError(
                "deploy non-jump reboot report gate counts conflict"
            )
        for value in (
        ...
        if (
            ...
            self.execution_state != DeployNonJumpRebootExecutionState.SUCCEEDED.value
            ...
            raise StatePersistenceError(
                "deploy non-jump reboot success report conflicts"
            )
...
def execute_deploy_non_jump_reboots(
    *,
    state_root: Path,
    cluster_name: str,
    operation_id: uuid.UUID,
...
) -> DeployNonJumpRebootExecutionReport:
    """Execute the exact immutable non-jump reboot order, serial one."""
    ...
    execution_store = DeployNonJumpRebootExecutionStore(paths, operation_id)
    evidence_store = DeployNonJumpRebootEvidenceStore(paths, operation_id)
    for path in (execution_store.path, evidence_store.path):
        validate_state_file(path, allow_missing=True)

    chain = _load_non_jump_context(paths, operation_id, lock=lock)
    reconciliation = _load_reconciliation(paths, operation_id, chain, lock=lock)
    candidates = _derive_candidates(chain, reconciliation)
    if not candidates:
        if any(
            path.exists()
            for path in (
    ...
    if execution is not None and execution.record.all_targets_completed:
        if evidence is None:
            raise StateConflictError(
                "completed deploy non-jump reboot evidence is unavailable"
            )
        return _build_report(context, execution, evidence, reused=True)
    if execution is not None and execution.record.state not in {
        DeployNonJumpRebootExecutionState.PREPARED,
        DeployNonJumpRebootExecutionState.SUCCEEDED,
    }:
        raise StateConflictError(
            "deploy non-jump reboot execution requires manual recovery and cannot retry"
        )

    service = AnsibleService(builder, runner)
    discovered = service.version(lock)
    if discovered != toolchain:
        raise StateConflictError("deploy non-jump reboot toolchain drifted")
    context = _reload_execution_context(
        paths,
        operation_id,
        lock=lock,
        builder=builder,
    ...
    if (
        execution is None
        or execution.record.state is DeployNonJumpRebootExecutionState.SUCCEEDED
    ):
        ...
        try:
            ...
            raise StatePersistenceError(
                "deploy non-jump reboot prepared intent persistence failed "
                "before invocation"
            ) from error

    assert execution is not None
    while True:
        if execution.record.state is DeployNonJumpRebootExecutionState.PREPARED:
            before = _reload_execution_context(
                paths,
                operation_id,
                lock=lock,
                builder=builder,
            ...
            if before.binding != context.binding:
                raise StateConflictError(
                    "deploy non-jump reboot state drifted before start"
                )
            ...
            try:
                ...
                raise StatePersistenceError(
                    "deploy non-jump reboot authorization consumption failed "
                    "before invocation"
                ) from error
            try:
                ...
                if observed_command_digest != scope.command_digest:
                    raise AnsibleResultError(
                        "deploy non-jump reboot command result identity conflicts"
                    )
            except KeyboardInterrupt:
                _persist_uncertain_or_raise(
                    execution_store,
                    execution,
                    DeployNonJumpRebootExecutionState.INTERRUPTED,
                    lock=lock,
                )
                raise AnsibleError(
                    "deploy non-jump reboot execution was interrupted; "
                ...
                raise AnsibleError(
                    "deploy non-jump reboot execution is uncertain; "
                    "manual recovery required"
                ) from error

            try:
                after = _reload_execution_context(
                    paths,
                    operation_id,
                    lock=lock,
                    builder=builder,
                ...
                raise StateConflictError(
                    "deploy non-jump reboot state changed after invocation; "
                    "manual recovery required"
                ) from error
            ...
            try:
                ...
                _persist_uncertain_or_raise(
                    execution_store,
                    execution,
                    DeployNonJumpRebootExecutionState.MALFORMED_RESULT,
                    lock=lock,
                )
                raise AnsibleError(
                    "deploy non-jump reboot result is malformed; "
                    "manual recovery required"
                ) from error
            try:
                ...
                raise StatePersistenceError(
                    "deploy non-jump reboot evidence persistence failed; "
                    "manual recovery required"
                ) from error
            ...
            try:
                ...
                raise StatePersistenceError(
                    "deploy non-jump reboot terminal persistence failed; "
                    "manual recovery required"
                ) from error
            if terminal_state is not DeployNonJumpRebootExecutionState.SUCCEEDED:
                raise AnsibleError(
                    "deploy non-jump reboot execution failed; manual recovery required"
                )
        ...
        try:
            ...
            raise StatePersistenceError(
                "deploy non-jump reboot next prepared intent failed before invocation"
            ) from error
    if evidence is None:
        raise StatePersistenceError(
            "deploy non-jump reboot completion evidence is missing"
        )
    _validate_prefix(context, execution, evidence)
    return _build_report(context, execution, evidence, reused=False)


def deploy_non_jump_reboot_execution_path(
    paths: StatePaths, operation_id: uuid.UUID
) -> Path:
    operation_id = _require_operation_id(operation_id)
    path = paths.operations / (
        f"{operation_id}{DEPLOY_NON_JUMP_REBOOT_EXECUTION_FILENAME_SUFFIX}"
    )
    if path.parent != paths.operations:
        raise StatePersistenceError(
            "deploy non-jump reboot execution path is not canonical"
        )
    return path


def deploy_non_jump_reboot_evidence_path(
    paths: StatePaths, operation_id: uuid.UUID
) -> Path:
    operation_id = _require_operation_id(operation_id)
    path = paths.operations / (
        f"{operation_id}{DEPLOY_NON_JUMP_REBOOT_EVIDENCE_FILENAME_SUFFIX}"
    )
    if path.parent != paths.operations:
        raise StatePersistenceError(
            "deploy non-jump reboot evidence path is not canonical"
        )
    return path


def deploy_non_jump_reboot_execution_id_from_filename(
    name: str,
) -> uuid.UUID | None:
    return _id_from_filename(name, DEPLOY_NON_JUMP_REBOOT_EXECUTION_FILENAME_SUFFIX)


def deploy_non_jump_reboot_evidence_id_from_filename(name: str) -> uuid.UUID | None:
    return _id_from_filename(name, DEPLOY_NON_JUMP_REBOOT_EVIDENCE_FILENAME_SUFFIX)
...
def _load_reconciliation(
    ...
    *,
    ...
    if not store.path.exists():
        raise StateConflictError(
            "deploy non-jump reboot execution requires reconciliation"
        )
    ...
    expected = _build_non_jump_reconciliation(
        chain,
        steps=_build_non_jump_steps(chain),
        created_at=stored.record.created_at,
    )
    if stored.record != expected:
        raise StateConflictError("deploy non-jump reboot reconciliation drifted")
    return stored


def _reload_execution_context(
    paths: StatePaths,
    operation_id: uuid.UUID,
    *,
    lock: ClusterLock,
    ...
    chain = _load_non_jump_context(paths, operation_id, lock=lock)
    reconciliation = _load_reconciliation(paths, operation_id, chain, lock=lock)
    return _load_execution_context(
        paths,
        operation_id,
        chain=chain,
        reconciliation=reconciliation,
...
def _load_execution_context(
    ...
    *,
    ...
    host_context = final_routes.post.post.base.host
    ...
    if (
        ...
        raise StateConflictError(
            "deploy non-jump reboot readiness, journal, or toolchain conflicts"
        )
    readiness = _reconstructed_readiness(planning.base)
    if readiness_binding_digest(readiness) != readiness_record.readiness_digest:
        raise StateConflictError("deploy non-jump reboot readiness is stale")
    ...
    for path, label in (
        ...
        if not path.exists():
            raise StateConflictError(
                f"deploy non-jump reboot execution requires {label}"
            )
    ...
    if (
        ...
        raise StateConflictError(
            "deploy non-jump reboot plan is blocked, stale, or drifted"
        )
    ...
    if (
        ...
        raise StateConflictError(
            "deploy non-jump reboot authorization is stale or consumed"
        )
    ...
    for target in plan.record.targets:
        ...
        if (
            ...
            or target.role not in _SAFE_ROLES
            ...
            raise StateConflictError(
                "deploy non-jump reboot target evidence is not safe"
            )
        ...
        request_digest = _digest_object(
            {
                ...
                "final_routes_evidence_digest": (target.final_routes_evidence_digest),
    ...
    if (
        ...
        or len({scope.target.stable_id for scope in scopes}) != len(scopes)
    ):
        raise StateConflictError(
            "deploy non-jump reboot execution scope is unavailable"
        )
    ...
    values: dict[str, object] = {
        ...
        "post_non_jump_reconciliation_record_digest": (
            reconciliation.record.record_digest
        ),
        "non_jump_base_os_authorization_artifact_digest": (
            plan_record.non_jump_base_os_authorization_artifact_digest
        ),
        "non_jump_base_os_execution_artifact_digest": (
            plan_record.non_jump_base_os_execution_artifact_digest
        ),
        "non_jump_base_os_execution_binding_digest": (
            plan_record.non_jump_base_os_execution_binding_digest
        ),
        "non_jump_base_os_evidence_artifact_digest": (
            plan_record.non_jump_base_os_evidence_artifact_digest
        ),
        "non_jump_base_os_evidence_digest": (
            plan_record.non_jump_base_os_evidence_digest
        ),
        "final_routes_execution_artifact_digest": (
            plan_record.final_routes_execution_artifact_digest
        ),
        "final_routes_evidence_artifact_digest": (
            plan_record.final_routes_evidence_artifact_digest
        ),
        "final_routes_evidence_digest": plan_record.final_routes_evidence_digest,
        "final_routes_reconciliation_artifact_digest": (
            plan_record.final_routes_reconciliation_artifact_digest
        ),
        "final_routes_reconciliation_record_digest": (
            plan_record.final_routes_reconciliation_record_digest
        ),
        ...
        "target_order_digest": plan_record.target_order_digest,
        "role_counts_digest": plan_record.role_counts_digest,
        "route_relationship_digest": plan_record.route_relationship_digest,
        "execution_scope_digest": execution_scope_digest,
        "binding_digest": "",
    }
    values["binding_digest"] = _binding_digest_from_values(values)
    binding = DeployNonJumpRebootExecutionBinding(**values)  # type: ignore[arg-type]
...
def _validate_prefix(
    ...
    if execution is None:
        if evidence is not None:
            raise StateConflictError(
                "deploy non-jump reboot evidence exists without intent"
            )
        return
    if execution.record.binding != context.binding:
        raise StateConflictError("deploy non-jump reboot execution provenance is stale")
    if evidence is not None and evidence.record.binding != context.binding:
        raise StateConflictError("deploy non-jump reboot evidence provenance is stale")
    for index, attempt in enumerate(execution.record.attempts):
        if index >= len(context.scopes):
            raise StateConflictError(
                "deploy non-jump reboot execution has extra attempts"
            )
        ...
        if (
            ...
            raise StateConflictError("deploy non-jump reboot execution scope conflicts")
    entries = evidence.record.entries if evidence is not None else ()
    if len(entries) > len(execution.record.attempts):
        raise StateConflictError("deploy non-jump reboot evidence prefix conflicts")
    for index, entry in enumerate(entries):
        attempt = execution.record.attempts[index]
        scope = context.scopes[index]
        if (
            attempt.state is DeployNonJumpRebootExecutionState.PREPARED
            ...
            raise StateConflictError(
                "deploy non-jump reboot semantic evidence conflicts"
            )
    ...
    allowed_started_extra = (
        1
        if execution.record.state is DeployNonJumpRebootExecutionState.STARTED
        and len(entries) == terminal_with_evidence + 1
        else 0
    )
    if len(entries) != terminal_with_evidence + allowed_started_extra:
        raise StateConflictError(
            "deploy non-jump reboot execution/evidence prefixes conflict"
        )
    if any(
        entry.status is not DeployRebootResultStatus.SUCCEEDED for entry in entries[:-1]
    ):
        raise StateConflictError(
            "deploy non-jump reboot execution continued after failure"
        )


def _persist_prepared(
    context: _ExecutionContext,
    store: DeployNonJumpRebootExecutionStore,
    current: StoredDeployNonJumpRebootExecution | None,
    scope: _RebootScope,
    *,
    lock: ClusterLock,
) -> StoredDeployNonJumpRebootExecution:
    ...
    attempt = DeployNonJumpRebootExecutionAttempt(
        ...
        source_digest=scope.source_digest,
        request_digest=scope.request_digest,
        state=DeployNonJumpRebootExecutionState.PREPARED,
        prepared_at=now,
        started_at=None,
        completed_at=None,
    ...
    if current.record.state is not DeployNonJumpRebootExecutionState.SUCCEEDED:
        raise StateConflictError(
            "deploy non-jump reboot cannot prepare after uncertain state"
        )
    record = replace(
        current.record,
...
def _persist_started(
    store: DeployNonJumpRebootExecutionStore,
    current: StoredDeployNonJumpRebootExecution,
    *,
    lock: ClusterLock,
) -> StoredDeployNonJumpRebootExecution:
    if current.record.state is not DeployNonJumpRebootExecutionState.PREPARED:
        raise StateConflictError(
            "deploy non-jump reboot start requires prepared intent"
        )
    now = _timestamp()
    attempt = replace(
        current.record.attempts[-1],
        state=DeployNonJumpRebootExecutionState.STARTED,
        started_at=now,
...
def _persist_uncertain_or_raise(
    ...
    *,
    ...
    try:
        ...
        raise StatePersistenceError(
            "deploy non-jump reboot uncertain outcome persistence failed; "
            "manual recovery required"
        ) from error
...
def _persist_evidence(
    context: _ExecutionContext,
    store: DeployNonJumpRebootEvidenceStore,
    current: StoredDeployNonJumpRebootEvidence | None,
    ...
    *,
    ...
    attempt = replace(
        ...
        manual_recovery_required=(
            state is not DeployNonJumpRebootExecutionState.SUCCEEDED
        ),
    )
    completed = current.record.completed_target_count + int(
        state is DeployNonJumpRebootExecutionState.SUCCEEDED
    )
    record = replace(
        current.record,
        generation=current.record.generation + 1,
...
def _semantic_entry(
    ...
    if (
        ...
        raise AnsibleResultError("deploy non-jump reboot result identity conflicts")
    ...
    return DeployNonJumpRebootEvidenceEntry(**values)  # type: ignore[arg-type]


def _terminal_state(
    entry: DeployNonJumpRebootEvidenceEntry, exit_code: int
) -> DeployNonJumpRebootExecutionState:
    if exit_code == 0 and entry.status is DeployRebootResultStatus.SUCCEEDED:
        return DeployNonJumpRebootExecutionState.SUCCEEDED
    if exit_code == 4 or entry.status is DeployRebootResultStatus.UNREACHABLE:
        return DeployNonJumpRebootExecutionState.UNREACHABLE
    return DeployNonJumpRebootExecutionState.FAILED
...
def _build_report(
    ...
    *,
    ...
    if (
        ...
        raise StateConflictError("deploy non-jump reboot execution is not complete")
    return DeployNonJumpRebootExecutionReport(
        operation_id=context.binding.operation_id,
        execution_artifact_state=(
            DeployNonJumpRebootArtifactState.REUSED
            if reused
            else DeployNonJumpRebootArtifactState.UPDATED
            if execution.record.generation > 3
            else DeployNonJumpRebootArtifactState.CREATED
        ),
...
def _not_required_report(
    ...
    return DeployNonJumpRebootExecutionReport(
        operation_id=operation_id,
        execution_artifact_state=DeployNonJumpRebootArtifactState.NOT_REQUIRED,
        evidence_artifact_state=DeployNonJumpRebootArtifactState.NOT_REQUIRED,
...
def _validate_execution_transition(
    current: DeployNonJumpRebootExecution,
    replacement: DeployNonJumpRebootExecution,
) -> None:
    if (
        ...
        not in {
            DeployNonJumpRebootExecutionState.PREPARED,
            DeployNonJumpRebootExecutionState.STARTED,
            DeployNonJumpRebootExecutionState.SUCCEEDED,
        }
    ):
        raise StatePersistenceError(
            "deploy non-jump reboot execution transition is invalid"
        )
    if current.state is DeployNonJumpRebootExecutionState.PREPARED:
        valid = (
            ...
            is DeployNonJumpRebootExecutionState.STARTED
        )
    elif current.state is DeployNonJumpRebootExecutionState.STARTED:
        valid = (
            len(replacement.attempts) == len(current.attempts)
            and replacement.attempts[:-1] == current.attempts[:-1]
            and replacement.attempts[-1].state
        ...
        valid = (
            ...
            and replacement.attempts[-1].state
            is DeployNonJumpRebootExecutionState.PREPARED
        )
    if not valid:
        raise StatePersistenceError(
            "deploy non-jump reboot execution transition conflicts"
        )


def _failure_state(error: AnsibleError) -> DeployNonJumpRebootExecutionState:
    if isinstance(error, AnsibleResultError):
        return DeployNonJumpRebootExecutionState.MALFORMED_RESULT
    cause = error.__cause__
    if isinstance(cause, ProcessTimeoutError):
        return DeployNonJumpRebootExecutionState.TIMED_OUT
...
def _binding_digest(binding: DeployNonJumpRebootExecutionBinding) -> str:
...
def _binding_digest_from_values(values: Mapping[str, object]) -> str:
    ...
    for name, field in DeployNonJumpRebootExecutionBinding.__dataclass_fields__.items():
...
def _entry_digest(entry: DeployNonJumpRebootEvidenceEntry) -> str:
...
def _entry_digest_from_values(values: Mapping[str, object]) -> str:
    value: dict[str, object] = {}
    for name, field in DeployNonJumpRebootEvidenceEntry.__dataclass_fields__.items():
        item = values.get(name, field.default)
        value[name] = (
            item.value
...
def _require_canonical_paths(paths: StatePaths) -> None:
    if (
        ...
        raise StatePersistenceError(
            "deploy non-jump reboot execution paths are not canonical"
        )
...
def _assert_operation_lock(lock: ClusterLock, paths: StatePaths) -> None:
    if not isinstance(lock, ClusterLock):
        raise StateLockError(
            "deploy non-jump reboot execution requires an acquired deploy lock"
        )
...
def _refuse_ambiguous_artifacts(paths: StatePaths, operation_id: uuid.UUID) -> None:
    try:
        ...
        raise StatePersistenceError(
            "cannot safely list deploy non-jump reboot execution artifacts"
        ) from error
    ...
    for suffix in (
        ...
        for entry in entries:
            ...
            if prefix != canonical and (
                ...
                raise StateConflictError(
                    "deploy non-jump reboot execution artifacts are ambiguous"
                )
...
__all__ = [
    ...
    "DeployNonJumpRebootEvidence",
    "DeployNonJumpRebootEvidenceEntry",
    "DeployNonJumpRebootEvidenceStore",
    "DeployNonJumpRebootExecution",
    "DeployNonJumpRebootExecutionAttempt",
    "DeployNonJumpRebootExecutionBinding",
    "DeployNonJumpRebootExecutionReport",
    "DeployNonJumpRebootExecutionState",
