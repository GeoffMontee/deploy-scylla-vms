"""Immutable reconciliation after mapped deploy ``manager-agent`` execution.

This subprocess-free owner reloads the exact canonical deploy chain, requires
complete ordered terminal success for the authorized Scylla scope, and
advances at most the immediate active historical mapping gate.  It never
authorizes or executes work and never changes the common journal or a prior
artifact.
"""
...
ANSIBLE_DEPLOY_POST_MANAGER_AGENT_STEP_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-post-manager-agent-step/v1"
)
ANSIBLE_DEPLOY_POST_MANAGER_AGENT_RECONCILIATION_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-post-manager-agent-reconciliation/v1"
)
ANSIBLE_DEPLOY_POST_MANAGER_AGENT_RECONCILIATION_REPORT_SCHEMA_VERSION = (
    "deploy-scylla-vms.ansible-deploy-post-manager-agent-reconciliation-report/v1"
)
DEPLOY_POST_MANAGER_AGENT_RECONCILIATION_FILENAME_SUFFIX = (
    ".ansible-deploy-post-manager-agent-reconciliation.json"
)

_OPERATION = "deploy"
_STAGE = "post-manager-agent-reconciliation"
_AGENT_PLAYBOOK = "manager-agent"
_AGENT_MAPPING = 16
_ORIGINAL_MAPPING_COUNT = 21
_NOT_STARTED = "not-started"
_UNAVAILABLE = "unavailable"
_AGENT_EVIDENCE_STATE = "manager-agent-bound"
...
class DeployPostManagerAgentArtifactState(StrEnum):
...
class DeployPostManagerAgentStep:
    """Full historical step identity plus one post-agent status."""
    ...
    blocker_digest: str
    step_digest: str
    schema_version: str = ANSIBLE_DEPLOY_POST_MANAGER_AGENT_STEP_SCHEMA_VERSION

    def __post_init__(self) -> None:
        definition = get_playbook(self.playbook)
        if (
            self.schema_version != ANSIBLE_DEPLOY_POST_MANAGER_AGENT_STEP_SCHEMA_VERSION
            or self.sequence < 1
            ...
            raise StatePersistenceError("post-manager-agent step identity conflicts")
        for digest in (
            self.target_digest,
            ...
            if digest is not None:
                validate_digest(digest, "post-manager-agent step digest")
    ...
    def _validate_status(self) -> None:
        ...
        if (
            ...
            if (
                ...
                raise StatePersistenceError(
                    "post-manager-agent authorization-required step conflicts"
                )
            return
        if self.status is DeployBaseOsReconciledStepStatus.ELIGIBLE:
            if (
                ...
                raise StatePersistenceError(
                    "post-manager-agent eligible step conflicts"
                )
            return
        if self.status is DeployBaseOsReconciledStepStatus.BLOCKED:
            if not self.blockers or self.evidence_digest is not None:
                raise StatePersistenceError("post-manager-agent blocked step conflicts")
            return
        if self.evidence_digest is not None:
            raise StatePersistenceError(
                "post-manager-agent not-performed step has evidence"
            )
    ...
    @classmethod
    def from_object(cls, value: Mapping[str, object]) -> DeployPostManagerAgentStep:
        require_exact_keys(value, set(cls.__dataclass_fields__), "post-agent step")
        evidence_digest = value["evidence_digest"]
        if evidence_digest is not None and not isinstance(evidence_digest, str):
            raise StatePersistenceError(
                "post-manager-agent evidence digest must be a string or null"
            )
        try:
            return cls(
            ...
            raise StatePersistenceError(
                "post-manager-agent step enum is invalid"
            ) from error


@dataclass(frozen=True, slots=True)
class DeployPostManagerAgentReconciliation:
    """Bounded immutable checkpoint after exact Manager agent installation."""
    ...
    steps: tuple[DeployPostManagerAgentStep, ...]
    step_count: int
    succeeded_count: int
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise StatePersistenceError(
                "post-manager-agent reconciliation identity conflicts"
            )
        ...
        for count in (
            ...
            _nonnegative_integer(count, "post-manager-agent count")
        if (
            ...
            raise StatePersistenceError(
                "post-manager-agent reconciliation counts conflict"
            )
        ...
        if len(mapping) != _ORIGINAL_MAPPING_COUNT or any(
            ...
            raise StatePersistenceError(
                "post-manager-agent immutable mapping conflicts"
            )
        agents = tuple(
        ...
        if (
            ...
            raise StatePersistenceError(
                "post-manager-agent reconciliation summary conflicts"
            )
        for digest in _digest_fields(self):
            validate_digest(digest, "post-manager-agent reconciliation digest")
    ...
    ) -> DeployPostManagerAgentReconciliation:
        require_exact_keys(
            value,
            set(cls.__dataclass_fields__),
            "post-manager-agent reconciliation",
        )
        integer_fields = {
            "generation",
            "journal_generation",
        ...
        try:
            for name in cls.__dataclass_fields__:
                ...
                if name in integer_fields:
                    ...
                    parsed[name] = tuple(
                        DeployPostManagerAgentStep.from_object(
                            _mapping(step, "post-manager-agent step")
                        )
            ...
            raise StatePersistenceError(
                "post-manager-agent reconciliation enum is invalid"
            ) from error
        return cls(**parsed)  # type: ignore[arg-type]


@dataclass(frozen=True, slots=True)
class StoredDeployPostManagerAgentReconciliation:
    record: DeployPostManagerAgentReconciliation
    artifact_digest: str


class DeployPostManagerAgentReconciliationStore:
    ...
    def __init__(
        ...
        *,
        ...
        self._path = deploy_post_manager_agent_reconciliation_path(paths, operation_id)
    ...
    def read(
        ...
        *,
        ...
        record = DeployPostManagerAgentReconciliation.from_object(value)
        if (
        ...
        return StoredDeployPostManagerAgentReconciliation(record, artifact_digest)
    ...
    def write_locked(
        ...
        *,
        ...
        StoredDeployPostManagerAgentReconciliation,
        DeployPostManagerAgentArtifactState,
    ]:
        _assert_operation_lock(lock, self._paths)
        ...
        if record.operation_id != self._operation_id:
            raise StatePersistenceError(
                "post-manager-agent reconciliation operation conflicts"
            )
        if self._path.exists():
            ...
            if current.record != record:
                raise StateConflictError(
                    "post-manager-agent reconciliation is immutable; "
                    "use a new operation"
                )
            return current, DeployPostManagerAgentArtifactState.REUSED
        digest = self._file.write(record.to_object(), expected_digest=None)
        return (
            StoredDeployPostManagerAgentReconciliation(record, digest),
            DeployPostManagerAgentArtifactState.CREATED,
        )


@dataclass(frozen=True, slots=True)
class DeployPostManagerAgentReconciliationReport:
    """Strict bounded post-manager-agent reconciliation projection."""

    operation_id: uuid.UUID
    artifact_state: DeployPostManagerAgentArtifactState
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise StatePersistenceError(
                "post-manager-agent reconciliation report conflicts"
            )
        for digest in _digest_fields(self):
            validate_digest(digest, "post-manager-agent report digest")
...
def reconcile_deploy_manager_agent_result(
    *,
    state_root: Path,
    cluster_name: str,
    operation_id: uuid.UUID,
    lock: ClusterLock,
) -> DeployPostManagerAgentReconciliationReport:
    """Bind exact Manager agent success and evaluate the immediate mapped gate."""
    ...
    store = DeployPostManagerAgentReconciliationStore(paths, operation_id)
...
def deploy_post_manager_agent_reconciliation_path(
    paths: StatePaths, operation_id: uuid.UUID
) -> Path:
    """Return the canonical operation-bound post-manager-agent path."""

    _require_canonical_paths(paths)
    operation_id = _require_operation_id(operation_id)
    path = paths.operations / (
        f"{operation_id}{DEPLOY_POST_MANAGER_AGENT_RECONCILIATION_FILENAME_SUFFIX}"
    )
    if path.parent != paths.operations:
        raise StatePersistenceError(
            "post-manager-agent reconciliation path is not canonical"
        )
    return path


def deploy_post_manager_agent_reconciliation_id_from_filename(
    name: str,
) -> uuid.UUID | None:
    suffix = DEPLOY_POST_MANAGER_AGENT_RECONCILIATION_FILENAME_SUFFIX
...
def _load_context(
    ...
    *,
    ...
    for path, label in (
        ...
        if not path.exists():
            raise StateConflictError(
                f"post-manager-agent reconciliation requires complete {label}"
            )
    ...
    if authorization.record != expected_authorization:
        raise StateConflictError(
            "post-manager-agent authorization or canonical scope drifted"
        )
...
def _validate_complete_execution(context: _ReconciliationContext) -> None:
    loaded = _loaded(
        context.authorization_context.monitoring.monitoring.manager.manager.chain.authorization_context
    )
    ...
    if (
        ...
        or execution.state is not DeployManagerAgentExecutionState.SUCCEEDED
        ...
        raise StateConflictError(
            "post-manager-agent reconciliation requires exact terminal success"
        )
    ...
    post = context.authorization_context.post_monitoring
    install = context.authorization_context.install.evidence
    post_install = context.authorization_context.post_install
    base_os = (
        context.authorization_context.monitoring.monitoring.manager.manager.base_os
    )
    ...
    if (
        ...
        or binding.post_monitoring_artifact_digest != post.artifact_digest
        or binding.post_monitoring_record_digest != post.record.record_digest
        or binding.post_monitoring_effective_plan_digest
        != _digest_object([step.to_object() for step in post.record.steps])
        ...
        raise StateConflictError(
            "post-manager-agent inventory, trust, readiness, source, catalog, "
            "authorization, or journal binding drifted"
        )
...
def _validate_scope_result(
    context: _ReconciliationContext,
    index: int,
    scope: DeployManagerAgentAuthorizationScope,
    ...
    expected_result = _digest_object(
        {
            "auth_token_configured": entry.auth_token_configured,
            "changed": entry.changed,
            "configuration_performed": entry.configuration_performed,
            "helper_slice_configured": entry.helper_slice_configured,
            "logical_id": entry.stable_id,
            "package_set_digest": _digest_object(
                {name: MANAGER_PACKAGE_VERSION for name in MANAGER_PACKAGES}
            ),
            ...
            "server_reachability": entry.server_reachability,
    ...
    prohibited = (
        ...
        entry.auth_token_configured,
        entry.helper_slice_configured,
    ...
    if (
        ...
        or attempt.package_version_digest != _digest_object(MANAGER_PACKAGE_VERSION)
        or attempt.package_provenance_digest != package.provenance_digest
        or attempt.state is not DeployManagerAgentExecutionState.SUCCEEDED
        ...
        or entry.changed != (entry.status is ManagerAgentStatus.INSTALLED)
        ...
        raise StateConflictError(
            "post-manager-agent package, service, execution, or prohibited-action "
            "evidence conflicts"
        )
...
def _expected_provenance_digest(
    ...
    install_matches = tuple(
        entry
        for entry in context.authorization_context.install.evidence.record.entries
        if entry.stable_id == scope.target_stable_id
    )
    if len(base_matches) != 1 or len(install_matches) != 1:
        raise StateConflictError(
            "post-manager-agent current prerequisite evidence is ambiguous"
        )
    ...
    if image_filter is None:
        raise StateConflictError(
            "post-manager-agent current Scylla image policy is absent"
        )
    ...
    if not isinstance(provenance, Mapping):
        raise StateConflictError(
            "post-manager-agent current package provenance is malformed"
        )
...
def _build_steps(
    context: _ReconciliationContext,
) -> tuple[DeployPostManagerAgentStep, ...]:
    prior_steps = context.authorization_context.post_monitoring.record.steps
    if tuple(sorted({step.mapping_sequence for step in prior_steps})) != tuple(
        range(1, _ORIGINAL_MAPPING_COUNT + 1)
    ):
        raise StateConflictError("post-manager-agent prior mapping is incomplete")
    remaining = tuple(
        sorted(
    ...
    if not remaining:
        raise StateConflictError(
            "post-manager-agent immediate next active mapping is unavailable"
        )
    ...
    result: list[DeployPostManagerAgentStep] = []
    for prior in prior_steps:
        ...
        if prior.mapping_sequence == _AGENT_MAPPING:
            if (
                ...
                raise StateConflictError(
                    "post-manager-agent executed mapping identity conflicts"
                )
            ...
            raise StateConflictError(
                "post-manager-agent later mapped gate already succeeded"
            )
            ...
            if status in {
                ...
                raise StateConflictError(
                    "post-manager-agent later mapped gate leapfrogged"
                )
...
def _validate_prior_step(prior: DeployPostMonitoringStackStep) -> None:
    mapping = OPERATION_PLAYBOOKS[_OPERATION]
    ...
    if (
        ...
        raise StateConflictError("post-manager-agent historical step identity drifted")


def _next_gate_ready(
    step: DeployPostMonitoringStackStep,
    context: _ReconciliationContext,
) -> bool:
    loaded = _loaded(
        context.authorization_context.monitoring.monitoring.manager.manager.chain.authorization_context
    ...
    base_hosts = {
        ...
        for entry in context.authorization_context.monitoring.monitoring.manager.manager.base_os.record.entries
...
def _next_gate_evidence_digest(
    ...
    install_entries = tuple(
        (entry.stable_id, entry.evidence_digest)
        for entry in context.authorization_context.install.evidence.record.entries
        if entry.stable_id in step.target_ids
    )
    predecessor_evidence = tuple(
        (prior.mapping_sequence, prior.playbook, prior.evidence_digest)
        for prior in context.authorization_context.post_monitoring.record.steps
    ...
    return _digest_object(
        {
            ...
            "manager_agent_evidence": [
                entry.evidence_digest for entry in context.evidence.record.entries
            ],
            "playbook": step.playbook,
            "predecessor_evidence": predecessor_evidence,
            "scylla_install_evidence": sorted(install_entries),
...
def _step_from_prior(
    prior: DeployPostMonitoringStackStep,
    *,
    status: DeployBaseOsReconciledStepStatus,
    evidence_state: str,
    evidence_digest: str | None,
    ...
    return DeployPostManagerAgentStep(**values)  # type: ignore[arg-type]
...
def _build_record(
    ...
    *,
    ...
    if not next_candidates:
        raise StateConflictError("post-manager-agent next mapped gate is unavailable")
    ...
    values: dict[str, object] = {
        ...
        "post_monitoring_artifact_digest": (
            context.authorization_context.post_monitoring.artifact_digest
        ),
        "post_monitoring_record_digest": (
            context.authorization_context.post_monitoring.record.record_digest
        ),
        "post_monitoring_effective_plan_digest": (
            binding.post_monitoring_effective_plan_digest
        ),
    ...
    return DeployPostManagerAgentReconciliation(**values)  # type: ignore[arg-type]


def _build_report(
    stored: StoredDeployPostManagerAgentReconciliation,
    *,
    state: DeployPostManagerAgentArtifactState,
) -> DeployPostManagerAgentReconciliationReport:
    record = stored.record
    return DeployPostManagerAgentReconciliationReport(
...
def _validate_original_mapping() -> None:
    ...
    if (
        ...
        raise StateConflictError(
            "post-manager-agent requires the immutable 21-position mapping"
        )


def _step_digest(step: DeployPostManagerAgentStep) -> str:
...
def _record_digest(record: DeployPostManagerAgentReconciliation) -> str:
...
def _record_digest_from_values(values: Mapping[str, object]) -> str:
    ...
    for (
        name,
        field,
    ) in DeployPostManagerAgentReconciliation.__dataclass_fields__.items():
...
def _refuse_ambiguous_or_later_artifacts(
    ...
    later_fragments = (
        ...
        ".ansible-deploy-post-monitoring-agent",
    ...
    try:
        ...
        raise StatePersistenceError(
            "cannot safely list post-manager-agent operation history"
        ) from error
    for entry in entries:
        if (
            entry.name.startswith(prefix)
            and "post-manager-agent-reconciliation" in entry.name
            and entry.name != canonical
        ):
            validate_state_file(entry)
            raise StateConflictError(
                "post-manager-agent reconciliation artifacts are ambiguous"
            )
        if entry.name.startswith(prefix) and any(
            ...
            validate_state_file(entry)
            raise StateConflictError(
                "post-manager-agent reconciliation refuses later-stage history"
            )
...
def _require_canonical_paths(paths: StatePaths) -> None:
    if (
        ...
        raise StatePersistenceError(
            "post-manager-agent reconciliation paths are not canonical"
        )
...
def _assert_operation_lock(lock: ClusterLock, paths: StatePaths) -> None:
    if not isinstance(lock, ClusterLock):
        raise StateLockError(
            "post-manager-agent reconciliation requires the matching held deploy lock"
        )
...
__all__ = [
    ...
    "deploy_post_manager_agent_reconciliation_path",
    "reconcile_deploy_manager_agent_result",
]
